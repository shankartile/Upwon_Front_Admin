...
import {
  HREASY_PACKAGE_BUTTON_STYLE_LABELS,
  HREASY_PACKAGE_BUTTON_STYLES,
  type CreateHreasyPackageTierInput,
  type HreasyPackageButtonStyle,
  type HreasyPackageTier,
...
const EMPTY: Form = {
  ...
  buttonLabel: '',
  buttonHref: '/demo',
  // The quiet treatment: a new card should not borrow the highlighted look
  // without being the highlighted card.
...
const toForm = (tier: HreasyPackageTier): Form => ({
  name: tier.name,
  slug: tier.slug,
  lead: tier.lead,
...
export default function HreasyPackageTierEditPage() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const body: CreateHreasyPackageTierInput = {
        name: form.name.trim(),
        slug: form.slug.trim(),
        lead: form.lead.trim(),
  ...
  return (
    <>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardBody>
              ...
              <Field
                label="Button style"
                hint="How the button is drawn. The highlighted card takes the filled one; the others are outlined."
              >
                <Select
                  value={form.buttonStyle}
                  onChange={(e) =>
                    patch({ buttonStyle: e.target.value as HreasyPackageButtonStyle })
                  }
                >
                  {HREASY_PACKAGE_BUTTON_STYLES.map((style) => (
                    <option key={style} value={style}>
                      {HREASY_PACKAGE_BUTTON_STYLE_LABELS[style]}
                    </option>
                  ))}
                </Select>
              </Field>
      ...
      */}
      {!isNew && id && <PackageFeaturesCard tierId={id} />}
