...
import { heroSection } from '../../../services/hreasyPageService';
...
import type { CreateHreasyHeroSlideInput, HreasyHeroSlide } from '../../../types/hreasyPage';

/**
 * Create / edit one HREasy hero slide, as a full page.
 *
...
const LIST_PATH = '/cms/products/hreasy/hero-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'hreasy_hero_slide';
...
const toForm = (slide: HreasyHeroSlide): Form => ({
...
export default function HreasyHeroSlideEditPage() {
  ...
  const [slide, setSlide] = useState<HreasyHeroSlide | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="HREasy hero slide" description="Could not load this slide." />
        <Card>
          <CardBody>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('hreasyHero', dimensions);
    if (problem) {
  ...
  const pickMobileImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('hreasyHeroMobile', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreateHreasyHeroSlideInput = {
      ...
      if (isNew) {
        ...
        toast.success('Slide updated', 'The public HREasy page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One slide of the HREasy page slider."
        actions={
          <Button
            variant="secondary"
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                label={HERO_IMAGE_SPECS.hreasyHero.label}
                error={form.imageError ?? undefined}
                hint={HERO_IMAGE_SPECS.hreasyHero.hint}
              >
                <ImagePicker
              ...
              <Field
                label={HERO_IMAGE_SPECS.hreasyHeroMobile.label}
                error={form.mobileImageError ?? undefined}
                hint={HERO_IMAGE_SPECS.hreasyHeroMobile.hint}
              >
                <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this slide? The public HREasy page will show the new content straight away.'
        }
