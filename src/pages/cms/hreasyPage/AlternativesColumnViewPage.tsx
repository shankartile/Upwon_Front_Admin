...
export default function HreasyAlternativesColumnViewPage() {
  ...
  /**
   * This column's answer on a row, or null when it has none.
   *
   * `?? null` rather than a falsiness check: `false` is a real answer here —
   * the cross — and collapsing it into "no cell" would hide half the grid.
   */
  const cellFor = (row: HreasyAlternativeRow): boolean | null =>
    row.cells.find((cell) => cell.columnId === column.id)?.flag ?? null;
                  ...
                  return (
                    <li key={row.id} className="px-5 py-3">
                      ...
                      {answer === null ? (
                        ...
                        <p className="mt-1 text-[13px] font-semibold text-orange-600 dark:text-orange-400">
                          ✓ Yes
                        </p>
                      ) : (
                        <p className="mt-1 text-[13px] text-charcoal-light dark:text-navy-300">
                          ✕ No
                        </p>
                      )}
                    </li>
