...
const NAME_MAX = 80;
/** COLUMN_DESCRIPTION_MAX in the server-side alternatives validator. */
...
const toForm = (column: HreasyAlternativesColumn): Form => ({
  name: column.name,
...
export default function HreasyAlternativesColumnEditPage() {
  ...
  const [column, setColumn] = useState<HreasyAlternativesColumn | null>(null);
  ...
  const save = async () => {
    setSaving(true);
    try {
      const body: CreateHreasyAlternativesColumnInput = {
        name: form.name.trim(),
        // Blank means no line under the name, which is null rather than ''.
      ...
      if (isNew) {
        ...
        toast.success('Column updated', 'The public POS page now shows this grid.');
      }
      navigate(LIST_PATH);
    } catch (error) {
        ...
        <Card>
          ...
          <CardBody>
            <FieldGrid cols={1}>
              ...
              <Field
                label="Sub-line"
                hint="The small grey line under the name. Optional — use it to name the products the column stands for, rather than putting a competitor's logo on the page."
              >
                <Input
                  value={form.description}
                  maxLength={DESCRIPTION_MAX}
                  placeholder="Keka, greytHR"
        ...
        description={
          stealsHighlight
            ...
            : isNew
              ...
              : 'Are you sure you want to update this column? The public POS page will show it straight away.'
        }
