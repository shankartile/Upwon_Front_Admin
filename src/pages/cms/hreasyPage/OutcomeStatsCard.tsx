...
import type { HreasyOutcomeStat } from '../../../types/hreasyPage';
...
/** MAX_HREASY_OUTCOME_STATS on the server. Shown before the 409 fires. */
const MAX_STATS = 3;

const BASE = '/cms/products/hreasy/outcomes-section/stories';

type Pending =
  | { kind: 'delete'; record: HreasyOutcomeStat }
  | { kind: 'status'; record: HreasyOutcomeStat; next: ContentStatus };

export default function OutcomeStatsCard({ storyId }: { storyId: string }) {
  const editPath = `${BASE}/${storyId}/stats`;

  const [stats, setStats] = useState<HreasyOutcomeStat[]>([]);
