...
import type { CreateHreasyPackageFeatureInput, HreasyPackageFeature } from '../../../types/hreasyPage';
...
const toForm = (feature: HreasyPackageFeature): Form => ({
  label: feature.label,
...
export default function HreasyPackageFeatureEditPage() {
  const { tierId, id } = useParams<{ tierId: string; id: string }>();
  const isNew = id === 'new';
  const navigate = useNavigate();
  const toast = useToast();

  const backPath = `/cms/products/hreasy/packages-section/tiers/${tierId}`;

  const [form, setForm] = useState<Form | null>(isNew ? { ...EMPTY } : null);
  const [feature, setFeature] = useState<HreasyPackageFeature | null>(null);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const body: CreateHreasyPackageFeatureInput = {
