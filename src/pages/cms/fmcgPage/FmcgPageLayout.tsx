...
export const FMCG_PAGE_BASE = '/cms/industries/fmcg-distribution';

export default function FmcgPageLayout() {
  ...
  return (
    <>
      <PageHeader
        title="FMCG Distribution Page"
        description="Content for the public UpWon FMCG Distribution industry page, section by section."
      />
      <Tabs
        tabs={SECTIONS.map((s) => ({ id: s.id, label: s.label }))}
        active={active}
        onChange={(id) => navigate(`${FMCG_PAGE_BASE}/${id}`)}
      />
      <div className="mt-5">
        <Outlet />
      </div>
