...
import { heroSection } from '../../../services/fmcgPageService';
...
import type { CreateFmcgHeroSlideInput, FmcgHeroSlide } from '../../../types/fmcgPage';

/**
 * Create / edit one FMCG hero slide, as a full page.
 *
...
const LIST_PATH = '/cms/industries/fmcg-distribution/hero-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'fmcg_hero_slide';
...
const toForm = (slide: FmcgHeroSlide): Form => ({
...
export default function FmcgHeroSlideEditPage() {
  ...
  const [slide, setSlide] = useState<FmcgHeroSlide | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="FMCG hero slide" description="Could not load this slide." />
        <Card>
          <CardBody>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('fmcgHero', dimensions);
    if (problem) {
  ...
  const pickMobileImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('fmcgHeroMobile', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreateFmcgHeroSlideInput = {
      ...
      if (isNew) {
        ...
        toast.success('Slide updated', 'The public FMCG Distribution page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One slide of the FMCG Distribution page slider."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this slide? The public FMCG Distribution page will show the new content straight away.'
        }
