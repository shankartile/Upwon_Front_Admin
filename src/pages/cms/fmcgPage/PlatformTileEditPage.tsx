...
import { platformSection as service } from '../../../services/fmcgPageService';
...
const LIST_PATH = '/cms/industries/fmcg-distribution/platform-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const ICON_ENTITY_TYPE = 'fmcg_platform_icon';
...
const toForm = (tile: FmcgPlatformTile): Form => ({
  label: tile.label,
  href: tile.href,
  status: tile.status,
...
export default function FmcgPlatformTileEditPage() {
  ...
  const [tile, setTile] = useState<FmcgPlatformTile | null>(null);
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('fmcgPlatformIcon', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateFmcgPlatformTileInput = {
      ...
      if (isNew) {
        ...
        toast.success('Tile updated', 'The FMCG Distribution page now shows this tile.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  const spec = HERO_IMAGE_SPECS.fmcgPlatformIcon;
  ...
  return (
    <>
      <PageHeader
        ...
        description="One tile in the connected platform grid on the FMCG Distribution page."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this tile? If it is saved as Active, it will join the grid on the FMCG Distribution page straight away.'
            : 'Are you sure you want to update this tile? The FMCG Distribution page will show the change straight away.'
        }
