...
import type { CreateSpicesAgroTrustLogoInput, SpicesAgroTrustLogo } from '../../../types/spicesAgroPage';
 ...
 * exists only to put a mark in the marquee, so one without an image would be a
 * gap rather than a variation.
 */

const LIST_PATH = '/cms/industries/spices-agro/trust-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const LOGO_ENTITY_TYPE = 'spices_agro_trust_logo';
 ...
 * optional: this marquee has no other text, so the name is the only thing a
...
const toForm = (logo: SpicesAgroTrustLogo): Form => ({
  alt: logo.alt,
  status: logo.status,
...
export default function SpicesAgroTrustLogoEditPage() {
  ...
  const [logo, setLogo] = useState<SpicesAgroTrustLogo | null>(null);
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateSpicesAgroTrustLogoInput = {
      ...
      if (isNew) {
        ...
        toast.success('Logo updated', 'The public Spices & Agro Processing page now shows this marquee.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One logo in the scrolling client marquee."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this logo? It will join the marquee straight away.'
            : 'Are you sure you want to update this logo? The public Spices & Agro Processing page will show it straight away.'
        }
