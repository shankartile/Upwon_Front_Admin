...
import type { SpicesAgroTrustLogo } from '../../../types/spicesAgroPage';

/**
 * The client logos - the marquee between the heading and the screenshot.
 *
...
const EDIT_PATH = '/cms/industries/spices-agro/trust-section/logos';

/** MAX_SPICES_AGRO_TRUST_LOGOS on the server. Shown as a hint before the 409 fires. */
const MAX_LOGOS = 24;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: SpicesAgroTrustLogo }
  | { kind: 'status'; record: SpicesAgroTrustLogo; next: ContentStatus };

export default function TrustLogosCard() {
  const [logos, setLogos] = useState<SpicesAgroTrustLogo[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The marquee under the heading. It scrolls, so the order is the order it starts in.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          disabled={atLimit}
          title={atLimit ? `The marquee holds at most ${MAX_LOGOS} logos` : undefined}
      ...
      <DataTable<SpicesAgroTrustLogo>
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first logo to take over the marquee from the site’s built-in set.'
        }
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the logo from the marquee.'
            : pending?.next === 'ACTIVE'
              ? 'This logo will start appearing in the marquee.'
