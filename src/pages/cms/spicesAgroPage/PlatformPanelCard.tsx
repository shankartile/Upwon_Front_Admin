...
import type { SpicesAgroPlatformPanel } from '../../../types/spicesAgroPage';

/**
 * The connected platform section's background illustration.
 *
 ...
 * alt text: the picture is decoration behind the section.
 */

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'spices_agro_platform_image';
...
const toForm = (panel: SpicesAgroPlatformPanel): Form => ({
...
export default function PlatformPanelCard() {
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('spicesAgroPlatformBackground', dimensions);
    if (problem) {
  ...
  return (
    <Card className="mt-6">
      <CardHeader
        ...
        subtitle="Behind the heading and the cards. It covers the whole section."
      />
      <CardBody>
        <div className="grid grid-cols-1 items-end gap-6 lg:grid-cols-[320px,1fr]">
          <Field
            label={HERO_IMAGE_SPECS.spicesAgroPlatformBackground.label}
            error={form.imageError ?? undefined}
            hint={HERO_IMAGE_SPECS.spicesAgroPlatformBackground.hint}
          >
            <ImagePicker
