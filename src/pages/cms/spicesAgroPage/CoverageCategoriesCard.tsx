...
/**
 * The industry coverage tiles - a round photo and a name each, six across on
 * wide screens.
 *
...
const EDIT_PATH = '/cms/industries/spices-agro/coverage-section/categories';

/** MAX_SPICES_AGRO_COVERAGE_CATEGORIES on the server. Shown as a hint before the 409 fires. */
const MAX_CATEGORIES = 18;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: SpicesAgroCoverageCategory }
  | { kind: 'status'; record: SpicesAgroCoverageCategory; next: ContentStatus };

export default function CoverageCategoriesCard() {
  const [categories, setCategories] = useState<SpicesAgroCoverageCategory[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The photo tiles under the heading, in order. At most {MAX_CATEGORIES}.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          disabled={atLimit}
          title={atLimit ? `The grid holds at most ${MAX_CATEGORIES} categories` : undefined}
      ...
      <DataTable<SpicesAgroCoverageCategory>
        data={categories}
        loading={loading}
        ...
        columns={[
          ...
          },
          {
            ...
            render: (row) => (
              ...
              <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border border-cream-300 bg-white dark:border-navy-800">
                {row.image ? (
                  <img
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the category and its tile.'
            : pending?.next === 'ACTIVE'
              ? 'This category’s tile will start appearing in the grid.'
              : 'This category’s tile will be removed from the live grid but kept here.'
        }
