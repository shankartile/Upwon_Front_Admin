...
import { trustSection as service } from '../../../services/spicesAgroPageService';
...
/**
 * The trust section's product panel: the screenshot under the logo marquee,
 * and what it shows.
 *
 * One record, so it is a form on the tab rather than a list with a form
 * behind it - the Engineering platform panel's arrangement, without its list
 * label.
 */

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'spices_agro_trust_panel';
...
const toForm = (panel: SpicesAgroTrustPanel): Form => ({
...
export default function TrustPanelCard() {
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('spicesAgroTrustPanel', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: UpsertSpicesAgroTrustPanelInput = {
        ...(imageFileId
      ...
      toast.success('Screenshot saved', 'The public Spices & Agro Processing page now shows it.');
    } catch (error) {
      toast.error('Could not save the screenshot', errorMessage(error));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Card className="mt-6">
      <CardHeader
        ...
        subtitle="The dashboard image under the logo marquee, and what it shows."
      />
      <CardBody>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[320px,1fr]">
          <Field
            label={HERO_IMAGE_SPECS.spicesAgroTrustPanel.label}
            error={form.imageError ?? undefined}
            hint={HERO_IMAGE_SPECS.spicesAgroTrustPanel.hint}
          >
            <ImagePicker
          ...
          <div className="space-y-4">
            ...
            >
              <Textarea
                ...
                placeholder="The UpWon dashboard showing procurement, production, inventory and sales…"
            ...
            <div className="flex justify-end">
              ...
              >
                {existed ? 'Save changes' : 'Create screenshot'}
              </Button>
            </div>
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void save()}
        title={existed ? 'Update the screenshot' : 'Create the screenshot'}
        description={
          existed
            ? 'Are you sure you want to update the screenshot? The public Spices & Agro Processing page will show it straight away.'
            : 'Are you sure you want to create the screenshot? It replaces the one the site ships with straight away.'
        }
...
function ImagePicker({
  ...
  return (
    <div className="space-y-2">
      {/* 3:2 and object-contain, the ratio the live section draws it at. */}
