...
/**
 * The core capabilities' artwork: the picture beside the row of cards, and
 * what it shows.
 *
...
const IMAGE_ENTITY_TYPE = 'qsr_franchise_capabilities_panel';
...
const toForm = (panel: QsrFranchiseCapabilitiesPanel): Form => ({
...
export default function CapabilitiesPanelCard() {
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('qsrFranchiseCapabilitiesArtwork', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: UpsertQsrFranchiseCapabilitiesPanelInput = {
        ...(imageFileId
      ...
      toast.success('Artwork saved', 'The public QSR & Franchise F&B page now shows it.');
    } catch (error) {
      toast.error('Could not save the artwork', errorMessage(error));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Card className="mt-6">
      <CardHeader
        title="Artwork"
        subtitle="The picture beside the row of cards, and what it shows."
      />
      <CardBody>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[240px,1fr]">
          <Field
            label={HERO_IMAGE_SPECS.qsrFranchiseCapabilitiesArtwork.label}
            error={form.imageError ?? undefined}
            hint={HERO_IMAGE_SPECS.qsrFranchiseCapabilitiesArtwork.hint}
          >
            <ImagePicker
          ...
          <div className="space-y-4">
            ...
            >
              <Textarea
                ...
                placeholder="A chef reviewing the UpWon dashboard, surrounded by procurement, production…"
      ...
      <ConfirmDialog
        ...
        description={
          existed
            ? 'Are you sure you want to update the artwork? The public QSR & Franchise F&B page will show it straight away.'
...
function ImagePicker({
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative aspect-[1041/1511] w-full overflow-hidden rounded-xl border border-dashed border-cream-400 bg-cream-100 dark:border-navy-700 dark:bg-navy-950/50">
        {preview ? (
