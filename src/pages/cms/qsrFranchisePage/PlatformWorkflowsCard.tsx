...
import type { QsrFranchisePlatformWorkflow } from '../../../types/qsrFranchisePage';

/**
 * The connected workflows - the grid of cards under the copy, five across on
 * desktop.
 *
...
const EDIT_PATH = '/cms/industries/qsr-franchise/platform-section/workflows';

/** MAX_QSR_FRANCHISE_PLATFORM_WORKFLOWS on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = 20;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: QsrFranchisePlatformWorkflow }
  | { kind: 'status'; record: QsrFranchisePlatformWorkflow; next: ContentStatus };

export default function PlatformWorkflowsCard() {
  const [workflows, setWorkflows] = useState<QsrFranchisePlatformWorkflow[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The grid of cards, five across on desktop, in order. At most {MAX_STATS}.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          title={
            atLimit
              ? `The grid holds at most ${MAX_STATS} workflows`
              : undefined
          }
      ...
      <DataTable<QsrFranchisePlatformWorkflow>
        data={workflows}
        loading={loading}
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the workflow from the grid.'
            : pending?.next === 'ACTIVE'
              ? 'This workflow will start appearing in the grid.'
              : 'This workflow will be removed from the live grid but kept here.'
        }
