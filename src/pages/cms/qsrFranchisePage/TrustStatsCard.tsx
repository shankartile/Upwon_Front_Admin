...
/**
 * The stat tiles - the figures in the mosaic under the logos, each with an
 * icon and a line about it.
 *
 * Capped on the server at the three places the mosaic has for them. The New
 * button goes dead at the cap.
 */

const EDIT_PATH = '/cms/industries/qsr-franchise/trust-section/stats';

/** MAX_QSR_FRANCHISE_TRUST_STATS on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = 3;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: QsrFranchiseTrustStat }
  | { kind: 'status'; record: QsrFranchiseTrustStat; next: ContentStatus };

export default function TrustStatsCard() {
  const [cards, setCards] = useState<QsrFranchiseTrustStat[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The figures in the mosaic under the logos, in order. The mosaic has places for{' '}
            {MAX_STATS}.
          </p>
        </div>
        <Button
          ...
          title={
            atLimit
              ? `The mosaic holds at most ${MAX_STATS} stat tiles`
              : undefined
          }
      ...
      <DataTable<QsrFranchiseTrustStat>
        data={cards}
        loading={loading}
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first stat to take over the tiles from the site’s built-in set.'
        }
        ...
        columns={[
          ...
          },
          {
            ...
            width: '76px',
            render: (row) => (
              // The round orange badge the tile draws beside its figure.
              <span
                className="flex h-10 w-10 items-center justify-center rounded-full bg-[#fff2e9] text-orange-500"
                title={row.icon}
              >
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the stat from the mosaic.'
            : pending?.next === 'ACTIVE'
              ? 'This stat will start appearing in the mosaic.'
              : 'This stat will be removed from the live mosaic but kept here.'
        }
