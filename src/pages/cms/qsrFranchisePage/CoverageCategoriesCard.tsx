...
/**
 * The industry coverage formats - a photo, an icon and a name each, in the
 * drifting row under the heading.
 *
 * Capped on the server. The New button goes dead at the cap.
 */

const EDIT_PATH = '/cms/industries/qsr-franchise/coverage-section/categories';

/** MAX_QSR_FRANCHISE_COVERAGE_CATEGORIES on the server. Shown as a hint before the 409 fires. */
const MAX_CATEGORIES = 18;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: QsrFranchiseCoverageCategory }
  | { kind: 'status'; record: QsrFranchiseCoverageCategory; next: ContentStatus };

export default function CoverageCategoriesCard() {
  const [categories, setCategories] = useState<QsrFranchiseCoverageCategory[]>([]);
  ...
  const runPending = async () => {
    ...
    try {
      if (pending.kind === 'delete') {
        ...
        toast.success(pending.next === 'ACTIVE' ? 'Format activated' : 'Format deactivated');
      }
      await load();
    } catch (error) {
    ...
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The cards in the drifting row under the heading, in order. At most {MAX_CATEGORIES}.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          disabled={atLimit}
          title={atLimit ? `The row holds at most ${MAX_CATEGORIES} formats` : undefined}
      ...
      {loadError && (
        <div className="mb-4 rounded-xl border border-orange-200 bg-orange-50 p-4 text-sm dark:border-orange-900/40 dark:bg-orange-900/10">
          <p className="font-medium text-orange-800 dark:text-orange-300">Could not load formats</p>
      ...
      <DataTable<QsrFranchiseCoverageCategory>
        data={categories}
        loading={loading}
        emptyTitle={isNarrowed ? 'No matching formats' : 'No formats yet'}
        emptyDescription={
          isNarrowed
            ? 'Try a different search term, or clear the status filter.'
            : 'Add the first format to take over the row from the site’s built-in set.'
        }
        ...
        columns={[
          ...
          },
          {
            ...
            width: '96px',
            render: (row) => (
              // object-cover - the crop the live card takes.
              <div className="flex h-12 w-16 items-center justify-center overflow-hidden rounded-md border border-cream-300 bg-white dark:border-navy-800">
                {row.image ? (
                  <img
          ...
          },
          {
            ...
            width: '72px',
            render: (row) => (
              // The badge the live card draws on the photo's edge.
              <span className="grid h-9 w-9 place-items-center rounded-full bg-[#fff2e9] text-orange-500">
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the format and its card.'
            : pending?.next === 'ACTIVE'
              ? 'This format’s card will start appearing in the row.'
              : 'This format’s card will be removed from the live row but kept here.'
        }
