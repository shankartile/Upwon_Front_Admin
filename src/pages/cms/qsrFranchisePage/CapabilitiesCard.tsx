...
/**
 * The core capabilities - the numbered cards in the drifting row beside the
 * artwork.
 *
 * Capped on the server. The New button goes dead at the cap. The number on
 * each card, and its icon's colour, follow its position here.
 */

const EDIT_PATH = '/cms/industries/qsr-franchise/capabilities-section/capabilities';

/** MAX_QSR_FRANCHISE_CAPABILITIES on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = 12;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: QsrFranchiseCapability }
  | { kind: 'status'; record: QsrFranchiseCapability; next: ContentStatus };

export default function CapabilitiesCard() {
  const [capabilities, setCapabilities] = useState<QsrFranchiseCapability[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The numbered cards in the row beside the artwork, in order — the first is 01. At most {MAX_STATS}.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          title={
            atLimit
              ? `The row holds at most ${MAX_STATS} capabilities`
              : undefined
          }
      ...
      <DataTable<QsrFranchiseCapability>
        data={capabilities}
        loading={loading}
        ...
        columns={[
          {
            ...
            width: '76px',
            render: (row) => (
              // Zero-padded, the way the card prints it.
          ...
          },
          {
            ...
            width: '72px',
            render: (row) => {
              // In the colour the live row draws it at this position.
              const palette = paletteFor(positionOf(capabilities.indexOf(row)));
              return (
                <span
