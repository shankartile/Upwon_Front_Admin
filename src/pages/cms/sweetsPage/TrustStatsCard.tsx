...
import { SWEETS_LIMITS, type SweetsTrustStat } from '../../../types/sweetsPage';

/**
 * The figures under the logo marquee - a number, what it counts and an icon
 * drawn above them.
 *
 * Capped at six on the server - the row is six across. The New button goes dead at
 * the cap rather than letting somebody write one the server will refuse.
 */

const EDIT_PATH = '/cms/industries/sweets-namkeen/trust-section/stats';

/** MAX_SWEETS_TRUST_STATS on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = SWEETS_LIMITS.trustStats;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: SweetsTrustStat }
  | { kind: 'status'; record: SweetsTrustStat; next: ContentStatus };

export default function TrustStatsCard() {
  const [stats, setStats] = useState<SweetsTrustStat[]>([]);
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The row under the customer logos: a number, what it counts and an icon
            above them. At most {MAX_STATS} — the row is six across.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          },
          {
            ...
            render: (row) => (
              <span className="flex items-center gap-2">
                <span className="grid h-8 w-8 place-items-center rounded-full bg-[#fbe3ce] text-[#8a4a12]">
