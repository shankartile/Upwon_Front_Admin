...
import { platformSection as service } from '../../../services/sweetsPageService';
...
const LIST_PATH = '/cms/industries/sweets-namkeen/platform-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const ICON_ENTITY_TYPE = 'sweets_platform_icon';
...
const toForm = (tile: SweetsPlatformTile): Form => ({
  label: tile.label,
  href: tile.href,
  status: tile.status,
...
export default function SweetsPlatformTileEditPage() {
  ...
  const [tile, setTile] = useState<SweetsPlatformTile | null>(null);
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('sweetsPlatformIcon', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateSweetsPlatformTileInput = {
      ...
      if (isNew) {
        ...
        toast.success('Tile updated', 'The Sweets & Namkeen page now shows this tile.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  const spec = HERO_IMAGE_SPECS.sweetsPlatformIcon;
  ...
  return (
    <>
      <PageHeader
        ...
        description="One tile in the connected platform grid on the Sweets & Namkeen page."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this tile? If it is saved as Active, it will join the grid on the Sweets & Namkeen page straight away.'
            : 'Are you sure you want to update this tile? The Sweets & Namkeen page will show the change straight away.'
        }
