...
import { heroSection } from '../../../services/sweetsPageService';
...
import type { CreateSweetsHeroSlideInput, SweetsHeroSlide } from '../../../types/sweetsPage';

/**
 * Create / edit one Sweets hero slide, as a full page.
 *
...
const LIST_PATH = '/cms/industries/sweets-namkeen/hero-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'sweets_hero_slide';
...
const toForm = (slide: SweetsHeroSlide): Form => ({
...
export default function SweetsHeroSlideEditPage() {
  ...
  const [slide, setSlide] = useState<SweetsHeroSlide | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="Sweets hero slide" description="Could not load this slide." />
        <Card>
          <CardBody>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('sweetsHero', dimensions);
    if (problem) {
  ...
  const pickMobileImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('sweetsHeroMobile', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreateSweetsHeroSlideInput = {
      ...
      if (isNew) {
        ...
        toast.success('Slide updated', 'The public Sweets & Namkeen page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One slide of the Sweets & Namkeen page slider."
        actions={
          <Button
            variant="secondary"
          ...
          <CardBody className="space-y-4">
            ...
            >
              <Input
                ...
                placeholder="Integrated ERP managing 50+ food & Sweets brands."
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                label={HERO_IMAGE_SPECS.sweetsHero.label}
                error={form.imageError ?? undefined}
                hint={HERO_IMAGE_SPECS.sweetsHero.hint}
              >
                <ImagePicker
              ...
              <Field
                label={HERO_IMAGE_SPECS.sweetsHeroMobile.label}
                error={form.mobileImageError ?? undefined}
                hint={HERO_IMAGE_SPECS.sweetsHeroMobile.hint}
              >
                <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this slide? The public Sweets & Namkeen page will show the new content straight away.'
        }
