...
export const SWEETS_PAGE_BASE = '/cms/industries/sweets-namkeen';

export default function SweetsPageLayout() {
  ...
  return (
    <>
      <PageHeader
        title="Sweets & Namkeen Page"
        description="Content for the public UpWon Sweets & Namkeen industry page, section by section."
      />
      <Tabs
        tabs={SECTIONS.map((s) => ({ id: s.id, label: s.label }))}
        active={active}
        onChange={(id) => navigate(`${SWEETS_PAGE_BASE}/${id}`)}
      />
      <div className="mt-5">
        <Outlet />
      </div>
