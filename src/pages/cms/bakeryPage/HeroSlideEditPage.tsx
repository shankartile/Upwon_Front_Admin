...
import { heroSection } from '../../../services/bakeryPageService';
...
import type { CreateBakeryHeroSlideInput, BakeryHeroSlide } from '../../../types/bakeryPage';

/**
 * Create / edit one Bakery hero slide, as a full page.
 *
...
const LIST_PATH = '/cms/industries/bakery-confectionery/hero-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'bakery_hero_slide';
...
const toForm = (slide: BakeryHeroSlide): Form => ({
...
export default function BakeryHeroSlideEditPage() {
  ...
  const [slide, setSlide] = useState<BakeryHeroSlide | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="Bakery hero slide" description="Could not load this slide." />
        <Card>
          <CardBody>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('bakeryHero', dimensions);
    if (problem) {
  ...
  const pickMobileImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('bakeryHeroMobile', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreateBakeryHeroSlideInput = {
      ...
      if (isNew) {
        ...
        toast.success('Slide updated', 'The public Bakery & Confectionery page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One slide of the Bakery & Confectionery page slider."
        actions={
          <Button
            variant="secondary"
          ...
          <CardBody className="space-y-4">
            ...
            >
              <Input
                ...
                placeholder="Integrated ERP managing 50+ food & FMCG brands."
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                label={HERO_IMAGE_SPECS.bakeryHero.label}
                error={form.imageError ?? undefined}
                hint={HERO_IMAGE_SPECS.bakeryHero.hint}
              >
                <ImagePicker
              ...
              <Field
                label={HERO_IMAGE_SPECS.bakeryHeroMobile.label}
                error={form.mobileImageError ?? undefined}
                hint={HERO_IMAGE_SPECS.bakeryHeroMobile.hint}
              >
                <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this slide? The public Bakery & Confectionery page will show the new content straight away.'
        }
