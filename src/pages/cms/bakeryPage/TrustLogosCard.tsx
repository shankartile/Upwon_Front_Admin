...
import { BAKERY_LIMITS, type BakeryTrustLogo } from '../../../types/bakeryPage';

/**
 * The customer logos in the trust section - the wall of bakery brands under
 * "Trusted by bakers across India".
 *
...
const EDIT_PATH = '/cms/industries/bakery-confectionery/trust-section/logos';

/** MAX_BAKERY_TRUST_LOGOS on the server. Shown as a hint before the 409 fires. */
const MAX_LOGOS = BAKERY_LIMITS.trustLogos;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: BakeryTrustLogo }
  | { kind: 'status'; record: BakeryTrustLogo; next: ContentStatus };

export default function TrustLogosCard() {
  const [logos, setLogos] = useState<BakeryTrustLogo[]>([]);
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The brand wall under the heading. At most {MAX_LOGOS}; lower positions come first.
          </p>
        </div>
        <Button
          variant="orange"
