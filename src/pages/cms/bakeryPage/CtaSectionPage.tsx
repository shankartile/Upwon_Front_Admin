...
import type { BakeryCtaSection, UpsertBakeryCtaSectionInput } from '../../../types/bakeryPage';
import CtaFeaturesCard from './CtaFeaturesCard';

/**
 * The Bakery & Confectionery page's closing band.
 *
 * One record, so this is the tab itself rather than a list with a form behind
 * it: the copy card at the top, the artwork pair and the buttons, then the
 * capability marks that sit under the buttons.
 *
 * Both crops are of the same picture. The desktop one keeps the copy over its
 * left side; the phone one over its top half - so those areas stay light.
 *
 * No footnote here, unlike the FMS band: this one closes on the marks instead
 * of a reassurance line.
 */

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'bakery_cta_image';
...
const toForm = (section: BakeryCtaSection): Form => ({
...
export default function BakeryCtaSectionPage() {
  ...
  const pickImage = async (which: 'desktop' | 'mobile', file: File) => {
    const slot: HeroImageVariant = which === 'desktop' ? 'bakeryCtaDesktop' : 'bakeryCtaMobile';
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: UpsertBakeryCtaSectionInput = {
        ...(desktopImageFileId
      ...
      setSubmitted(false);
      toast.success('Closing band saved', 'The public Bakery & Confectionery page now shows this content.');
    } catch (error) {
  ...
  return (
    <>
          ...
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                ...
                hint={HERO_IMAGE_SPECS.bakeryCtaDesktop.hint}
              >
                <ArtworkPicker
              ...
              <Field
                ...
                hint={HERO_IMAGE_SPECS.bakeryCtaMobile.hint}
              >
                <ArtworkPicker
      ...
      {/*
       * Not flush with the page bottom as on the other bands: the marks follow
       * it, and they save on their own, so the bar belongs to the band alone.
       */}
      ...
      <ConfirmDialog
        ...
        description={
          existed
            ? 'Are you sure you want to update this band? The public Bakery & Confectionery page will show the new content straight away.'
