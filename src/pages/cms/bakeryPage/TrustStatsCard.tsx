...
import { BAKERY_LIMITS, type BakeryTrustStat } from '../../../types/bakeryPage';

/**
 * The figures under the brand wall - the row of numbers with a round
 * illustration beside each.
 *
 * Capped at five on the server, because that is what the row holds. The New
 * button goes dead at the cap rather than letting somebody write a sixth figure
 * that would never render.
 */

const EDIT_PATH = '/cms/industries/bakery-confectionery/trust-section/stats';

/** MAX_BAKERY_TRUST_STATS on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = BAKERY_LIMITS.trustStats;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: BakeryTrustStat }
  | { kind: 'status'; record: BakeryTrustStat; next: ContentStatus };

export default function TrustStatsCard() {
  const [stats, setStats] = useState<BakeryTrustStat[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The row under the brand wall. At most {MAX_STATS} — that is what it holds. One
            can be featured as the highlighted cell.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          title={
            atLimit
              ? `The row holds at most ${MAX_STATS} figures`
              : undefined
          }
      ...
      <DataTable<BakeryTrustStat>
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first figure — until then this section is hidden on the public page.'
        }
        ...
        columns={[
          ...
          },
          {
            ...
            width: '72px',
            render: (row) => (
              // A circle, matching the row: the illustration is optional, so an
              // empty one shows a muted placeholder rather than a gap.
              <span className="grid h-9 w-9 place-items-center overflow-hidden rounded-full border border-cream-300 bg-white dark:border-navy-800">
                {row.icon ? (
                  <img
          ...
          },
          {
            ...
            render: (row) =>
              row.isFeatured ? (
                ...
                <span className="text-xs text-charcoal-light dark:text-navy-300">—</span>
              ),
          },
          {
            key: 'updatedAt',
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This removes the figure from the row and the public page. The record is kept for audit, but no longer appears in this list.'
            : pending?.next === 'ACTIVE'
