...
import { BAKERY_LIMITS, type BakeryPlatformTile } from '../../../types/bakeryPage';

/**
 * The connected platform - "One Connected Platform for Every Bakery &
...
const EDIT_PATH = '/cms/industries/bakery-confectionery/platform-section';

/** MAX_BAKERY_PLATFORM_TILES on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = BAKERY_LIMITS.platformTiles;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: BakeryPlatformTile }
  | { kind: 'status'; record: BakeryPlatformTile; next: ContentStatus };

export default function BakeryPlatformSectionPage() {
  const [entries, setEntries] = useState<BakeryPlatformTile[]>([]);
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ...
            : pending?.next === 'ACTIVE'
              ? 'This tile will start appearing in the Bakery & Confectionery page grid.'
