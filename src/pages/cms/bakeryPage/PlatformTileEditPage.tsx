...
const ICON_ENTITY_TYPE = 'bakery_platform_icon';
...
const toForm = (tile: BakeryPlatformTile): Form => ({
  label: tile.label,
  href: tile.href,
  status: tile.status,
...
export default function BakeryPlatformTileEditPage() {
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('bakeryPlatformIcon', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateBakeryPlatformTileInput = {
      ...
      if (isNew) {
        ...
        toast.success('Tile updated', 'The Bakery & Confectionery page now shows this tile.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  const spec = HERO_IMAGE_SPECS.bakeryPlatformIcon;
  ...
  return (
    <>
      <PageHeader
        ...
        description="One tile in the connected platform grid on the Bakery & Confectionery page."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this tile? If it is saved as Active, it will join the grid on the Bakery & Confectionery page straight away.'
            : 'Are you sure you want to update this tile? The Bakery & Confectionery page will show the change straight away.'
        }
