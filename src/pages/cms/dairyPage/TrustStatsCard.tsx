...
import { trustSection as service } from '../../../services/dairyPageService';
import { DEFAULT_PAGE_SIZE } from '../../../config/constants';
import { errorMessage } from '../../../lib/http';
import { assetUrl } from '../../../lib/assetUrl';
import { fmtDate, relativeTime } from '../../../lib/formatters';
import { STATUS_LABELS, type ContentStatus } from '../../../types/homePage';
import { DAIRY_LIMITS, type DairyTrustStat } from '../../../types/dairyPage';

/**
 * The figures the trust card rotates through - a number, what it counts and
 * the photograph shown with it.
 *
 * Capped on the server. The New button goes dead at the cap rather than
 * letting somebody write one the server will refuse.
 */

const EDIT_PATH = '/cms/industries/dairy/trust-section/stats';

/** MAX_DAIRY_TRUST_STATS on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = DAIRY_LIMITS.trustStats;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: DairyTrustStat }
  | { kind: 'status'; record: DairyTrustStat; next: ContentStatus };

export default function TrustStatsCard() {
  const [stats, setStats] = useState<DairyTrustStat[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The card rotates through these, in order: a number, what it counts and its
            photograph. At most {MAX_STATS}.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          title={
            atLimit
              ? `The card holds at most ${MAX_STATS} figures`
              : undefined
          }
      ...
      <DataTable<DairyTrustStat>
        ...
        columns={[
          ...
          },
          {
            ...
            width: '96px',
            render: (row) => (
              <div className="flex h-12 w-14 items-center justify-center overflow-hidden rounded-lg border border-cream-300 bg-white dark:border-navy-800">
                {row.image ? (
                  <img
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This removes the figure from the card and the public page. The record is kept for audit, but no longer appears in this list.'
            : pending?.next === 'ACTIVE'
              ? 'This figure will start appearing in the card.'
