...
const LIST_PATH = '/cms/industries/dairy/platform-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const ICON_ENTITY_TYPE = 'dairy_platform_icon';
...
const toForm = (tile: DairyPlatformTile): Form => ({
  label: tile.label,
  href: tile.href,
  status: tile.status,
...
export default function DairyPlatformTileEditPage() {
  ...
  const [tile, setTile] = useState<DairyPlatformTile | null>(null);
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('dairyPlatformIcon', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateDairyPlatformTileInput = {
      ...
      if (isNew) {
        ...
        toast.success('Tile updated', 'The Dairy & Ice Cream page now shows this tile.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  const spec = HERO_IMAGE_SPECS.dairyPlatformIcon;
  ...
  return (
    <>
      <PageHeader
        ...
        description="One tile in the connected platform grid on the Dairy & Ice Cream page."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this tile? If it is saved as Active, it will join the grid on the Dairy & Ice Cream page straight away.'
            : 'Are you sure you want to update this tile? The Dairy & Ice Cream page will show the change straight away.'
        }
