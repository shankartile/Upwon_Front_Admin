 ...
 * form is only the category: its photograph and its name.
 *
...
const IMAGE_ENTITY_TYPE = 'dairy_coverage_image';
...
const toForm = (item: DairyCoverageItem): Form => ({
  label: item.label,
  status: item.status,
...
export default function DairyCoverageItemEditPage() {
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('dairyCoverage', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateDairyCoverageItemInput = {
      ...
      if (isNew) {
        ...
        toast.success('Category updated', 'The Dairy & Ice Cream page now shows this category.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  const spec = HERO_IMAGE_SPECS.dairyCoverage;
  ...
  return (
    <>
      <PageHeader
        ...
        description="One category in the industry coverage row on the Dairy & Ice Cream page."
        actions={
          <Button
            variant="secondary"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
        <Card>
          <CardHeader
            title="The category"
            subtitle="The photograph and the name over it are all the card shows."
          />
          <CardBody>
            <FieldGrid>
              <Field
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this category? If it is saved as Active, it will join the grid on the Dairy & Ice Cream page straight away.'
            : 'Are you sure you want to update this category? The Dairy & Ice Cream page will show the change straight away.'
        }
...
function ImagePicker({
  ...
  return (
    <div className="flex items-start gap-4">
      {/* object-cover, matching the card: the photograph fills it. */}
