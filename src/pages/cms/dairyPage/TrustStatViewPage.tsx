...
import type { DairyTrustStat } from '../../../types/dairyPage';

/**
 * One figure from the trust card, read-only.
 *
 * Not the edit form with its inputs disabled: a form full of greyed-out boxes
 * reads as "broken" rather than "not yours to change". This shows the figure
 * with its photograph, as the card draws them.
 */
...
export default function DairyTrustStatViewPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [stat, setStat] = useState<DairyTrustStat | null>(null);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
        <Card>
          <CardHeader title="The figure" subtitle="The number, what it counts and its photograph." />
          <CardBody className="space-y-4">
            {stat.image ? (
              <img
                src={assetUrl(stat.image) ?? undefined}
                alt=""
                className="aspect-[5/4] w-full max-w-sm rounded-2xl object-cover"
              />
            ) : (
              <p className="text-sm text-orange-700 dark:text-orange-400">
                The photograph is missing - this figure is left off the public page until one is chosen.
              </p>
            )}
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-4">
              ...
              <ReadOnlyField label="Position in card" value={String(stat.displayOrder + 1)} />
              <ReadOnlyField
                label="Last updated"
