 ...
 * the copy above it or the benefits below.
 *
 * Null from the server means "never authored": the section then draws its
 * benefits without an image beside them.
 */

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'dairy_benefits_panel';

/** Field rules, mirroring the server-side dairy benefits panel validator. */
...
const toForm = (panel: DairyBenefitsPanel): Form => ({
  image: {
    ...EMPTY_IMAGE,
...
export default function BenefitsPanelCard() {
      ...
      <Card className="mb-6">
        <CardBody>
          <p className="text-sm text-orange-700 dark:text-orange-400">
            Could not load the section image — {loadError}
          </p>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('dairyBenefitsPanel', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: UpsertDairyBenefitsPanelInput = {
        ...(imageFileId
      ...
      toast.error('Could not save the section image', errorMessage(error));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      <Card className="mb-6">
        <CardHeader
          title="Section image"
          subtitle="The image beside the benefits. Drawn at its natural ratio."
        />
        <CardBody className="space-y-4">
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
            <Field
              ...
              hint={HERO_IMAGE_SPECS.dairyBenefitsPanel.hint}
            >
              <PanelPicker
            ...
            >
              <Input
                ...
                placeholder="Milk and ingredients, a bottling line, ice cream scoops and a cold storage warehouse"
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void save()}
        title={existed ? 'Update the section image' : 'Set the section image'}
        description={
          existed
...
/** Picks the section image. Holds the File until save, so cancelling orphans nothing. */
function PanelPicker({
  ...
  return (
    <div className="space-y-2">
      {/* At its natural ratio, as the live section draws it beside the benefits. */}
      <div className="relative max-w-md overflow-hidden rounded-xl border border-dashed border-cream-400 bg-cream-100 dark:border-navy-700 dark:bg-navy-950/50">
        {preview ? (
          <img src={preview} alt="" className="block h-auto w-full" />
        ) : (
          <span className="grid aspect-[1200/800] w-full place-items-center">
