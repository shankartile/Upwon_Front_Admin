...
import type { CreateDairyTrustLogoInput, DairyTrustLogo } from '../../../types/dairyPage';
...
const LOGO_ENTITY_TYPE = 'dairy_trust_logo';

/**
 * Field rules, mirroring the server-side dairy trust section validator.
 *
...
const toForm = (logo: DairyTrustLogo): Form => ({
  alt: logo.alt,
  status: logo.status,
...
export default function DairyTrustLogoEditPage() {
  ...
  const [logo, setLogo] = useState<DairyTrustLogo | null>(null);
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateDairyTrustLogoInput = {
      ...
      if (isNew) {
        ...
        toast.success(
          'Logo updated',
          'The public Dairy & Ice Cream page now shows this wall.',
        );
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One logo in the trust section's marquee."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this logo? The public Dairy & Ice Cream page will show it straight away.'
        }
