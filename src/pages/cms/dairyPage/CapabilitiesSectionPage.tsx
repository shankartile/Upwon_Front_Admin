...
import { DAIRY_LIMITS, type DairyCapabilityCard } from '../../../types/dairyPage';
import CapabilitiesPanelCard from './CapabilitiesPanelCard';

/**
 * Core capabilities - "Built to Simplify. Connected to Scale. Designed for
 * Dairy & Ice Cream Operations."
 *
 * Three things to edit, in the order the page draws them: the copy, authored
 * once; the collage beside the cards, one record with its own Save; and the
 * cards themselves - an icon, a title and the sentence under it, one row per
 * card, searched and paged by the database, with the form on its own page.
 */

const EDIT_PATH = '/cms/industries/dairy/capabilities-section';

/** MAX_DAIRY_CAPABILITY_CARDS on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = DAIRY_LIMITS.capabilityCards;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: DairyCapabilityCard }
  | { kind: 'status'; record: DairyCapabilityCard; next: ContentStatus };

export default function CapabilitiesSectionPage() {
  const [entries, setEntries] = useState<DairyCapabilityCard[]>([]);
    ...
    <>
      <SectionCopyCard
        ...
        placeholders={{
          ...
          heading:
            'Built to Simplify. Connected to Scale. **Designed for Dairy & Ice Cream Operations.**',
          subtext:
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            One card per capability - an icon, a title and the sentence under it. Lower
            display orders come first, and a card&apos;s position sets its number (01, 02…).
          </p>
        </div>
        <Button
          variant="orange"
          ...
          disabled={atLimit}
          title={atLimit ? `The section holds at most ${MAX_ENTRIES} capabilities` : undefined}
      ...
      <DataTable<DairyCapabilityCard>
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first capability — until then this section is hidden on the public page.'
        }
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This removes the capability from the section and the public page. The record is kept for audit, but no longer appears in this list.'
            : pending?.next === 'ACTIVE'
              ? 'This capability will start appearing in the Dairy & Ice Cream page core capabilities.'
              : 'This capability will be removed from the live section but kept here.'
        }
