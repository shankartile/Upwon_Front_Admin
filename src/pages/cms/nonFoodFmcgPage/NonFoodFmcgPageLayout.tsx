...
export const NON_FOOD_FMCG_PAGE_BASE = '/cms/industries/non-food-fmcg';

export default function NonFoodFmcgPageLayout() {
  ...
  return (
    <>
      <PageHeader
        title="Non-Food FMCG Page"
        description="Content for the public UpWon Non-Food FMCG industry page, section by section."
      />
      <Tabs
        tabs={SECTIONS.map((s) => ({ id: s.id, label: s.label }))}
        active={active}
        onChange={(id) => navigate(`${NON_FOOD_FMCG_PAGE_BASE}/${id}`)}
      />
      <div className="mt-5">
        <Outlet />
      </div>
