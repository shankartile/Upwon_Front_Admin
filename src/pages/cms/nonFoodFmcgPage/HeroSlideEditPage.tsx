...
import type { CreateNonFoodFmcgHeroSlideInput, NonFoodFmcgHeroSlide } from '../../../types/nonFoodFmcgPage';

/**
 * Create / edit one Non-Food FMCG hero slide, as a full page.
 *
...
const LIST_PATH = '/cms/industries/non-food-fmcg/hero-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'non_food_fmcg_hero_slide';
...
const toForm = (slide: NonFoodFmcgHeroSlide): Form => ({
...
export default function NonFoodFmcgHeroSlideEditPage() {
  ...
  const [slide, setSlide] = useState<NonFoodFmcgHeroSlide | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="Non-Food FMCG hero slide" description="Could not load this slide." />
        <Card>
          <CardBody>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('nonFoodFmcgHero', dimensions);
    if (problem) {
  ...
  const pickMobileImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('nonFoodFmcgHeroMobile', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreateNonFoodFmcgHeroSlideInput = {
      ...
      if (isNew) {
        ...
        toast.success('Slide updated', 'The public Non-Food FMCG page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One slide of the Non-Food FMCG page slider."
        actions={
          <Button
            variant="secondary"
          ...
          <CardBody className="space-y-4">
            ...
            >
              <Input
                ...
                placeholder="Integrated ERP managing 50+ food & Non-Food FMCG brands."
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                label={HERO_IMAGE_SPECS.nonFoodFmcgHero.label}
                error={form.imageError ?? undefined}
                hint={HERO_IMAGE_SPECS.nonFoodFmcgHero.hint}
              >
                <ImagePicker
              ...
              <Field
                label={HERO_IMAGE_SPECS.nonFoodFmcgHeroMobile.label}
                error={form.mobileImageError ?? undefined}
                hint={HERO_IMAGE_SPECS.nonFoodFmcgHeroMobile.hint}
              >
                <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this slide? The public Non-Food FMCG page will show the new content straight away.'
        }
