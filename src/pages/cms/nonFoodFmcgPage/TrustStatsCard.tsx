...
import { NON_FOOD_FMCG_LIMITS, type NonFoodFmcgTrustStat } from '../../../types/nonFoodFmcgPage';

/**
 * The figures under the logo marquee - a number, what it counts and a line
 * explaining it.
 *
 * Capped at six on the server - two rows of three. The New button goes dead at
 * the cap rather than letting somebody write one the server will refuse.
 */

const EDIT_PATH = '/cms/industries/non-food-fmcg/trust-section/stats';

/** MAX_NON_FOOD_FMCG_TRUST_STATS on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = NON_FOOD_FMCG_LIMITS.trustStats;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: NonFoodFmcgTrustStat }
  | { kind: 'status'; record: NonFoodFmcgTrustStat; next: ContentStatus };

export default function TrustStatsCard() {
  const [stats, setStats] = useState<NonFoodFmcgTrustStat[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The row under the customer logos: a number, what it counts and a line
            explaining it. At most {MAX_STATS} — two rows of three.
          </p>
        </div>
        <Button
          variant="orange"
      ...
      <DataTable<NonFoodFmcgTrustStat>
        data={stats}
        loading={loading}
        ...
        columns={[
          ...
          },
          {
            ...
            header: 'Explanation',
            render: (row) => (
              <p className="line-clamp-2 text-xs text-charcoal-light dark:text-navy-300">
                {row.description}
              </p>
            ),
          },
