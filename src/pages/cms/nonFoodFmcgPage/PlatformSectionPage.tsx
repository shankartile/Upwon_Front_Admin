...
import { NON_FOOD_FMCG_LIMITS, type NonFoodFmcgPlatformTile } from '../../../types/nonFoodFmcgPage';

/**
 * The connected platform - "Five Connected Solutions. One Smarter Non-Food FMCG
 * Operation."
 *
...
const EDIT_PATH = '/cms/industries/non-food-fmcg/platform-section';

/** MAX_NON_FOOD_FMCG_PLATFORM_TILES on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = NON_FOOD_FMCG_LIMITS.platformTiles;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: NonFoodFmcgPlatformTile }
  | { kind: 'status'; record: NonFoodFmcgPlatformTile; next: ContentStatus };

export default function NonFoodFmcgPlatformSectionPage() {
  const [entries, setEntries] = useState<NonFoodFmcgPlatformTile[]>([]);
  ...
  return (
    <>
      ...
      <DataTable<NonFoodFmcgPlatformTile>
        data={entries}
        loading={loading}
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ...
            : pending?.next === 'ACTIVE'
              ? 'This tile will start appearing in the Non-Food FMCG page grid.'
