...
/**
 * The dashboard image between the coverage copy and its categories.
 *
 ...
 * the copy above it or the categories below.
 *
 * Null from the server means "never authored": the section then shows its
 * categories straight after the copy, with no dashboard.
 */

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'non_food_fmcg_coverage_panel';

/** Field rules, mirroring the server-side non-food FMCG coverage panel validator. */
...
const toForm = (panel: NonFoodFmcgCoveragePanel): Form => ({
  image: {
    ...EMPTY_IMAGE,
...
export default function CoveragePanelCard() {
  ...
  const imageProblem = useMemo(() => {
    ...
    if (!form.image.file && !form.image.fileId && !form.image.url) {
      return 'Choose a dashboard image.';
    }
    return null;
  }, [form]);
      ...
      <Card className="mb-6">
        <CardBody>
          <p className="text-sm text-orange-700 dark:text-orange-400">
            Could not load the dashboard image — {loadError}
          </p>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('nonFoodFmcgCoverageDashboard', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: UpsertNonFoodFmcgCoveragePanelInput = {
        ...(imageFileId
      ...
      toast.success(
        ...
        'The public Non-Food FMCG page now shows this picture.',
      );
    } catch (error) {
      toast.error('Could not save the dashboard image', errorMessage(error));
    } finally {
      setSaving(false);
    }
  };
    ...
    <>
      <Card className="mb-6">
        <CardHeader
          ...
          subtitle="The picture between the copy above and the categories below. Drawn full width at its natural height."
        />
        <CardBody className="space-y-4">
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
            <Field
              ...
              hint={HERO_IMAGE_SPECS.nonFoodFmcgCoverageDashboard.hint}
            >
              <DashboardPicker
            ...
            >
              <Input
                ...
                placeholder="UpWon dashboard showing product, channel and distributor performance"
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void save()}
        title={existed ? 'Update the dashboard image' : 'Set the dashboard image'}
        description={
          existed
            ? 'Are you sure you want to update this image? The public Non-Food FMCG page will show it straight away.'
...
/** Picks the dashboard image. Holds the File until save, so cancelling orphans nothing. */
function DashboardPicker({
  ...
  return (
    <div className="space-y-2">
      {/* Full width at its natural height, as the live section draws it. */}
      <div className="relative overflow-hidden rounded-xl border border-dashed border-cream-400 bg-cream-100 dark:border-navy-700 dark:bg-navy-950/50">
        {preview ? (
          <img src={preview} alt="" className="block h-auto w-full" />
        ) : (
          <span className="grid aspect-[1200/740] w-full place-items-center">
