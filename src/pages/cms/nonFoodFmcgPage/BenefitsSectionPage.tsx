...
import { NON_FOOD_FMCG_LIMITS, type NonFoodFmcgBenefitItem } from '../../../types/nonFoodFmcgPage';

/**
 * Benefits - "Greater Control Over Every Channel. Better Visibility Across
 * Every Product Movement."
 *
 * Two things to edit: the copy over the grid, authored once, and the benefit
 * cards themselves - an icon over a short label, one row per card, searched
 * and paged by the database, with the form on its own page.
 */

const EDIT_PATH = '/cms/industries/non-food-fmcg/benefits-section';

/** MAX_NON_FOOD_FMCG_BENEFIT_ITEMS on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = NON_FOOD_FMCG_LIMITS.benefitItems;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: NonFoodFmcgBenefitItem }
  | { kind: 'status'; record: NonFoodFmcgBenefitItem; next: ContentStatus };

export default function NonFoodFmcgBenefitsSectionPage() {
  const [entries, setEntries] = useState<NonFoodFmcgBenefitItem[]>([]);
    ...
    <>
      <SectionCopyCard
        ...
        placeholders={{
          eyebrow: 'BENEFITS',
          heading:
            'Greater Control Over Every Channel. **Better Visibility Across Every Product Movement.**',
          subtext:
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            One card per benefit - an icon over a short label. Lower display orders come
            first.
          </p>
        </div>
        <Button
      ...
      <DataTable<NonFoodFmcgBenefitItem>
        data={entries}
        loading={loading}
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ...
            : pending?.next === 'ACTIVE'
              ? 'This benefit will start appearing in the Non-Food FMCG page benefits grid.'
              : 'This benefit will be removed from the live grid but kept here.'
        }
