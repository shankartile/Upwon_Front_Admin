...
import { heroSection } from '../../../services/erpPageService';
...
import type { CreateErpHeroSlideInput, ErpHeroSlide } from '../../../types/erpPage';

/**
 * Create / edit one ERP hero slide, as a full page.
 *
...
const LIST_PATH = '/cms/products/erp/hero-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'erp_hero_slide';
...
const toForm = (slide: ErpHeroSlide): Form => ({
...
export default function ErpHeroSlideEditPage() {
  ...
  const [slide, setSlide] = useState<ErpHeroSlide | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="ERP hero slide" description="Could not load this slide." />
        <Card>
          <CardBody>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('erpHero', dimensions);
    if (problem) {
  ...
  const pickMobileImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('erpHeroMobile', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreateErpHeroSlideInput = {
      ...
      if (isNew) {
        ...
        toast.success('Slide updated', 'The public ERP page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One slide of the ERP page slider."
        actions={
          <Button
            variant="secondary"
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                label={HERO_IMAGE_SPECS.erpHero.label}
                error={form.imageError ?? undefined}
                hint={HERO_IMAGE_SPECS.erpHero.hint}
              >
                <ImagePicker
              ...
              <Field
                label={HERO_IMAGE_SPECS.erpHeroMobile.label}
                error={form.mobileImageError ?? undefined}
                hint={HERO_IMAGE_SPECS.erpHeroMobile.hint}
              >
                <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this slide? The public ERP page will show the new content straight away.'
        }
