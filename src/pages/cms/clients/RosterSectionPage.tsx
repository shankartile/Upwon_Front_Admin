...
/**
 * Roster admin - "Trusted by India's Leading Food & FMCG Brands." on the
 * public /clients page: the section copy card on top, then one row per logo
 * in the scrolling marquee.
 */

const EDIT_PATH = '/cms/clients/roster-section';

/** MAX_CLIENTS_ROSTER_LOGOS on the server. Shown as a hint before the 409 fires. */
const MAX_LOGOS = 24;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: ClientsRosterLogo }
  | { kind: 'status'; record: ClientsRosterLogo; next: ContentStatus };

export default function ClientsRosterSectionPage() {
  const [logos, setLogos] = useState<ClientsRosterLogo[]>([]);
    ...
    <>
      <SectionCopyCard
        ...
        placeholders={{
          ...
          subtext:
            'Bakery chains, dairy brands, sweet houses, beverage makers, QSR networks and FMCG distributors.',
        }}
      />
      ...
      <div className="mb-4 flex justify-end">
        <Button
          ...
          disabled={atLimit}
          title={atLimit ? `The roster holds at most ${MAX_LOGOS} logos` : undefined}
      ...
      <DataTable<ClientsRosterLogo>
        data={logos}
        loading={loading}
        emptyTitle={isNarrowed ? 'No matching logos' : 'No roster logos yet'}
        emptyDescription={
          isNarrowed
            ? 'Try a different search term, or clear the status filter.'
            : 'Add the first logo to start the marquee.'
        }
        ...
        columns={[
          ...
          },
          {
            ...
            width: '140px',
            render: (row) => {
              const src = siteAssetUrl(row.image);
              return src ? (
                // object-contain, as the marquee draws it.
                <span className="flex h-12 w-28 items-center justify-center rounded-md border border-cream-300 bg-white p-1.5 dark:border-navy-800">
                  <img src={src} alt={row.name} className="max-h-full max-w-full object-contain" />
                </span>
              ) : (
                <span
                  className="flex h-12 w-28 items-center justify-center rounded-md border border-dashed border-cream-400 text-charcoal-light dark:border-navy-700 dark:text-navy-300"
                  title="No image on this logo"
                >
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the logo from the roster.'
            : pending?.next === 'ACTIVE'
              ? 'This logo will start appearing in the live marquee.'
              : 'This logo will be removed from the live marquee but kept here.'
        }
