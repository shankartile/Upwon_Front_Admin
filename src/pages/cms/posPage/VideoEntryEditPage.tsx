...
import type { CreatePosVideoEntryInput, PosVideoEntry } from '../../../types/posPage';
...
const LIST_PATH = '/cms/products/pos/video-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const VIDEO_ENTITY_TYPE = 'pos_video';
...
const toForm = (entry: PosVideoEntry): Form => ({
  status: entry.status,
...
export default function PosVideoEntryEditPage() {
  ...
  const [entry, setEntry] = useState<PosVideoEntry | null>(null);
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreatePosVideoEntryInput = {
        status: form.status,
        ...(form.source === 'upload'
          ? { videoFileId }
      ...
      if (isNew) {
        ...
        toast.success('Video updated', 'The public POS page now plays this clip.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
              ...
              <FieldGrid cols={1}>
                ...
                >
                  <Input
                    value={form.videoUrl}
                    maxLength={URL_MAX}
                    placeholder="https://cdn.example.com/pos-tour.mp4"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this video? If it is saved as Active, the POS page will play it straight away.'
            : 'Are you sure you want to update this video? The public POS page will show the change straight away.'
        }
