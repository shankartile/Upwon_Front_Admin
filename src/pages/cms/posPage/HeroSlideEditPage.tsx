...
import { heroSection } from '../../../services/posPageService';
...
import type { CreatePosHeroSlideInput, PosHeroSlide } from '../../../types/posPage';

/**
 * Create / edit one POS hero slide, as a full page.
 *
...
const LIST_PATH = '/cms/products/pos/hero-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'pos_hero_slide';
...
const toForm = (slide: PosHeroSlide): Form => ({
...
export default function PosHeroSlideEditPage() {
  ...
  const [slide, setSlide] = useState<PosHeroSlide | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="POS hero slide" description="Could not load this slide." />
        <Card>
          <CardBody>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('posHero', dimensions);
    if (problem) {
  ...
  const pickMobileImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('posHeroMobile', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreatePosHeroSlideInput = {
      ...
      if (isNew) {
        ...
        toast.success('Slide updated', 'The public POS page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One slide of the POS page slider."
        actions={
          <Button
            variant="secondary"
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                label={HERO_IMAGE_SPECS.posHero.label}
                error={form.imageError ?? undefined}
                hint={HERO_IMAGE_SPECS.posHero.hint}
              >
                <ImagePicker
              ...
              <Field
                label={HERO_IMAGE_SPECS.posHeroMobile.label}
                error={form.mobileImageError ?? undefined}
                hint={HERO_IMAGE_SPECS.posHeroMobile.hint}
              >
                <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this slide? The public POS page will show the new content straight away.'
        }
