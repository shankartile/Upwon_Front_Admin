...
import type { CreatePosSecurityLogoInput, PosSecurityLogo } from '../../../types/posPage';
...
const LIST_PATH = '/cms/products/pos/security-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const LOGO_ENTITY_TYPE = 'pos_security_logo';

/**
 * Field rules, mirroring the server-side security section validator.
 *
...
const toForm = (logo: PosSecurityLogo): Form => ({
  alt: logo.alt,
  status: logo.status,
...
export default function PosSecurityLogoEditPage() {
  ...
  const [logo, setLogo] = useState<PosSecurityLogo | null>(null);
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreatePosSecurityLogoInput = {
      ...
      if (isNew) {
        ...
        toast.success('Logo updated', 'The public POS page now shows this wall.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One mark pinned to the integration sphere."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this logo? The public POS page will show it straight away.'
        }
