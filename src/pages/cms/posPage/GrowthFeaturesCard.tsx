...
import type { PosGrowthFeature } from '../../../types/posPage';

/**
 * One tier's tick list.
 *
 * Not paginated, unlike the tier list: the list is capped at twelve, so a
 * pager would only ever show one page.
 */

/** MAX_POS_GROWTH_FEATURES on the server. Shown before the 409 fires. */
const MAX_FEATURES = 12;

const BASE = '/cms/products/pos/growth-section/tiers';

type Pending =
  | { kind: 'delete'; record: PosGrowthFeature }
  | { kind: 'status'; record: PosGrowthFeature; next: ContentStatus };

export default function GrowthFeaturesCard({ tierId }: { tierId: string }) {
  const editPath = `${BASE}/${tierId}/features`;

  const [features, setFeatures] = useState<PosGrowthFeature[]>([]);
