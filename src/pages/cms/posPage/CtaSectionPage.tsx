...
import { ctaSection as service } from '../../../services/posPageService';
...
import type { PosCtaSection, UpsertPosCtaSectionInput } from '../../../types/posPage';

/**
 * The POS page's closing band.
 *
 * One record, so this is the tab itself rather than a list with a form behind
 * it: the copy card at the top, the artwork pair, and the buttons.
 *
 * Both crops are of the same picture. The desktop one keeps its photograph on
 * the left third with a light panel on the right that the copy sits in; the
 * phone one is text-safe all over, because the copy is centred there.
 */

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'pos_cta_image';

/**
 * Field rules, mirroring the server-side POS closing band validator.
 *
...
const RULES = {
  ...
  footnote: { label: 'Footnote', min: 0, max: 300, required: false },
  note: { label: 'Handwritten note', min: 0, max: 160, required: false },
} as const;
...
const toForm = (section: PosCtaSection): Form => ({
...
export default function PosCtaSectionPage() {
  ...
  const errors = useMemo(() => {
    ...
    return {
      ...
      footnote: validateField('footnote', form.footnote),
  ...
  const pickImage = async (which: 'desktop' | 'mobile', file: File) => {
    const slot: HeroImageVariant = which === 'desktop' ? 'posCtaDesktop' : 'posCtaMobile';
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: UpsertPosCtaSectionInput = {
        ...(desktopImageFileId
        ...
        // Blank turns the note off, which is a real choice - the band reads
        // correctly without it, and phones never draw it at all.
      ...
      toast.success('Closing band saved', 'The public POS page now shows this content.');
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
          ...
          <Card>
            ...
            <CardBody className="space-y-4">
              <FieldGrid>
                <Field label={RULES.primaryLabel.label} error={errorFor('primaryLabel')}>
                  <Input
                    ...
                    placeholder="Talk to a Franchise Specialist"
              ...
              <FieldGrid>
                ...
                >
                  <Input
                    ...
                    placeholder="See How This Fits Your Outlet Network"
              ...
              <Field
                ...
                hint={`The reassurance line under the buttons. ${form.footnote.trim().length}/${RULES.footnote.max}`}
              >
                <Input
                  value={form.footnote}
                  maxLength={RULES.footnote.max}
                  placeholder="No obligation. 30 minutes about your business, not a sales pitch."
                  aria-invalid={!!errorFor('footnote')}
                  onBlur={() => setTouched((t) => ({ ...t, footnote: true }))}
              ...
              <Field
                ...
                error={errorFor('note')}
                hint="The note the background arrow points at, in the top right. Optional, and only drawn from 1024px up - phones never show it. Each line is a new line."
              >
                <Textarea
                  rows={2}
                  value={form.note}
                  maxLength={RULES.note.max}
                  placeholder={'Your Counter.\nOur Focus.'}
                  aria-invalid={!!errorFor('note')}
                  onBlur={() => setTouched((t) => ({ ...t, note: true }))}
                  onChange={(e) => patch({ note: e.target.value })}
                />
              </Field>

              {/* The note as the band draws it, so the line break is visible. */}
              {form.note.trim() && (
                <div className="rounded-2xl border border-cream-300 bg-white p-5 dark:border-navy-800 dark:bg-navy-950/50">
                  <p
                    className="text-2xl leading-[1.15] text-charcoal"
                    style={{
                      fontFamily:
                  ...
                  <span className="mt-1.5 block h-[3px] w-[92px] rounded-full bg-orange-500" />
                </div>
              )}
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader
              title="The artwork"
              subtitle="Two crops of one picture. Both optional — without them the band keeps its cream ground."
            />
            <CardBody className="space-y-5">
              <Field
                label="Desktop artwork"
                error={form.desktop.error ?? undefined}
                hint={HERO_IMAGE_SPECS.posCtaDesktop.hint}
              >
                <ArtworkPicker
                  aspect="aspect-[16/6]"
              ...
              <Field
                ...
                hint={HERO_IMAGE_SPECS.posCtaMobile.hint}
              >
                <ArtworkPicker
                  aspect="aspect-[9/16]"
      ...
      <ConfirmDialog
        ...
        description={
          existed
            ? 'Are you sure you want to update this band? The public POS page will show the new content straight away.'
            : 'Are you sure you want to create this band? It replaces the one the site ships with straight away.'
        }
