...
import type { PosOutcomeStory } from '../../../types/posPage';
...
const EDIT_PATH = '/cms/products/pos/outcomes-section/stories';

/** MAX_POS_OUTCOME_STORIES on the server. Shown before the 409 fires. */
const MAX_STORIES = 8;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: PosOutcomeStory }
  | { kind: 'status'; record: PosOutcomeStory; next: ContentStatus };

export default function OutcomeStoriesCard() {
  const [stories, setStories] = useState<PosOutcomeStory[]>([]);
  ...
  return (
    <section className="mt-8">
      ...
      <DataTable<PosOutcomeStory>
        ...
        columns={[
          ...
          },
          {
            ...
            width: '112px',
            render: (row) => (
              <div className="h-10 w-20 overflow-hidden rounded-lg border border-cream-300 bg-navy-950 dark:border-navy-800">
                {row.photo ? (
                  <img
            ...
            render: (row) => (
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-charcoal-light dark:text-navy-300">
                  {row.personName} · {row.personCompany}
                </p>
              </div>
            ),
          },
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? `This permanently removes “${pending.record.name}” from the carousel.`
            : pending?.next === 'ACTIVE'
