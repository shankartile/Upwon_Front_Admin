...
import type { CreatePosProofStatInput, PosProofStat } from '../../../types/posPage';

/**
 * Create / edit one figure in the proof row, as a full page.
 *
 * `:id` of 'new' means create - the same sentinel the other CMS edit screens
 * use.
 *
 * Two fields where the FMS page's equivalent has four. This strip draws one
 * divided row in the page's own accent, with the source folded into the label
 * ("faster billing (Kaka Halwai)"), so there is no colour to pick and no
 * second line to write. Offering either would be offering something the
 * section cannot render.
 */
...
/** Field rules, mirroring the server-side POS proof validator. */
const RULES = {
...
const toForm = (stat: PosProofStat): Form => ({
  icon: stat.icon,
  value: stat.value,
  label: stat.label,
...
export default function PosProofStatEditPage() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const body: CreatePosProofStatInput = {
        icon: form.icon,
      ...
      if (isNew) {
        ...
        toast.success('Figure updated', 'The public POS page now shows this number.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One column of the divided row beside the brand wall."
        actions={
          <Button
            variant="secondary"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-4">
              ...
              <Field
                ...
                hint="The row has no second line, so name the source here too — the section's own heading promises the number is real."
              >
                <Input
                  value={form.label}
                  maxLength={RULES.label.max}
                  placeholder="orders processed daily (U2 Cake)"
              ...
              {/* The column as the row draws it. */}
          ...
          <Card>
            <CardHeader title="Icon" subtitle="Drawn in the tinted circle above the number." />
            <CardBody>
              <IconPicker
                value={form.icon}
                options={icons}
          ...
          <Card>
            ...
            <CardBody>
              <div className="flex items-center gap-3">
                <span
                  className="h-8 w-8 shrink-0 rounded-full border border-cream-300 dark:border-navy-800"
                  style={{ background: ACCENT }}
                />
                <p className="text-xs text-charcoal-light dark:text-navy-300">
                  The whole row is drawn in the POS page's accent, so there is nothing to pick
                  here. The FMS page's figures each carry their own colour because that design
                  draws them as separate cards.
                </p>
              </div>
            </CardBody>
          </Card>
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this figure? The public POS page will show it straight away.'
        }
