...
import type { PosSecurityLogo } from '../../../types/posPage';

/**
 * The marks pinned to the integration sphere.
 *
 * Its own list rather than the home page's, and deliberately not identical to
 * it: the shipped sphere carries fifteen of the home page's sixteen marks and
 * leaves out the attendance device, because this one sits under a claim about
 * what a counter connects to.
 */

const EDIT_PATH = '/cms/products/pos/security-section/logos';

/** MAX_POS_SECURITY_LOGOS on the server. Shown as a hint before the 409 fires. */
const MAX_LOGOS = 24;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: PosSecurityLogo }
  | { kind: 'status'; record: PosSecurityLogo; next: ContentStatus };

export default function SecurityLogosCard() {
  const [logos, setLogos] = useState<PosSecurityLogo[]>([]);
