 ...
 * it.", "You decide."
 *
 * An icon and a phrase, nothing else - which is why these have their own
 * small list rather than sharing the badges' shape.
 */

const EDIT_PATH = '/cms/products/pos/security-section/assurances';

/** MAX_POS_SECURITY_ASSURANCES on the server. Shown before the 409 fires. */
const MAX_ASSURANCES = 6;
...
type Pending =
  | { kind: 'delete'; record: PosSecurityAssurance }
  | { kind: 'status'; record: PosSecurityAssurance; next: ContentStatus };

export default function SecurityAssurancesCard() {
  const [assurances, setAssurances] = useState<PosSecurityAssurance[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { rows, meta } = await service.assurances.list({
  ...
  const runPending = async () => {
    ...
    try {
      if (pending.kind === 'delete') {
        await service.assurances.remove(pending.record.id);
        toast.success('Assurance deleted');
      } else {
        await service.assurances.setStatus(pending.record.id, pending.next);
        toast.success(
          pending.next === 'ACTIVE' ? 'Assurance activated' : 'Assurance deactivated',
        );
      }
      await load();
    } catch (error) {
  ...
  const atLimit = total >= MAX_ASSURANCES;
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The short row at the foot of the data strip. At most {MAX_ASSURANCES} — they sit on
            one line, so a long list stops reading as a list.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          disabled={atLimit}
          title={atLimit ? `The row holds at most ${MAX_ASSURANCES} assurances` : undefined}
      ...
      <DataTable<PosSecurityAssurance>
        data={assurances}
        loading={loading}
        emptyTitle={isNarrowed ? 'No matching assurances' : 'No assurances yet'}
        emptyDescription={
          isNarrowed
            ? 'Try a different search term, or clear the status filter.'
            : 'Add the first assurance to take the row over from the site’s built-in set.'
        }
        ...
        columns={[
          {
            ...
            render: (row) => (
              <span className="tabular-nums text-charcoal-light dark:text-navy-300">
                {positionOf(assurances.indexOf(row)) + 1}
              </span>
            ),
          },
          {
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the phrase from the row.'
            : pending?.next === 'ACTIVE'
              ? 'This assurance will start appearing in the row.'
              : 'This assurance will be removed from the live row but kept here.'
        }
