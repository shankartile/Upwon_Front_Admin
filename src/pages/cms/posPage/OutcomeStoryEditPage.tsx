...
import { outcomesSection as service } from '../../../services/posPageService';
...
import type { CreatePosOutcomeStoryInput, PosOutcomeStory } from '../../../types/posPage';
...
const LIST_PATH = '/cms/products/pos/outcomes-section';

/** Entity types these uploads are tagged with, to make them publicly servable. */
const LOGO_ENTITY_TYPE = 'pos_outcome_logo';
const PHOTO_ENTITY_TYPE = 'pos_outcome_photo';

/** Field rules, mirroring the server-side outcomes validator. */
const RULES = {
  name: { label: 'Network name', min: 2, max: 160 },
  slug: { label: 'Slug', min: 2, max: 80 },
  quote: { label: 'Quote', min: 10, max: 600 },
  personName: { label: 'Who said it', min: 2, max: 160 },
  personCompany: { label: 'Where they work', min: 1, max: 160 },
  linkLabel: { label: 'Link label', min: 2, max: 120 },
...
const EMPTY: Form = {
  ...
  personName: '',
  personCompany: '',
  linkLabel: 'Watch the case study',
  linkHref: '/demo',
  status: 'ACTIVE',
  displayOrder: '',
  logo: { ...EMPTY_MEDIA },
...
const toForm = (story: PosOutcomeStory): Form => ({
  name: story.name,
  slug: story.slug,
  quote: story.quote,
  ...
  photo: {
    fileId: story.photoFileId,
    url: story.photoUrl,
    file: null,
    preview: assetUrl(story.photo) ?? null,
    error: null,
  },
});
...
function validateField(name: TextFieldName, raw: string): string | null {
  ...
  // Mirrors pos_outcome_stories_slug_format_check.
  if (name === 'slug' && !/^[a-z0-9]+(-[a-z0-9]+)*$/.test(value)) {
    return 'Use lowercase letters, digits and single hyphens — like u2-cake.';
  }
...
export default function PosOutcomeStoryEditPage() {
  ...
  const [form, setForm] = useState<Form | null>(
    isNew ? { ...EMPTY, logo: { ...EMPTY_MEDIA }, photo: { ...EMPTY_MEDIA } } : null,
  );
  const [story, setStory] = useState<PosOutcomeStory | null>(null);
  ...
  const errors = useMemo(() => {
    ...
    return {
      ...
      personCompany: validateField('personCompany', form.personCompany),
  ...
  const mediaProblem = useMemo(() => {
    ...
    if (!has(form.logo)) return 'A story needs a brand mark — choose one to continue.';
    if (!has(form.photo)) return 'A story needs a background photo — choose one to continue.';
    return null;
  }, [form]);

  const hasErrors =
    Object.values(errors).some(Boolean) ||
    Boolean(form?.logo.error) ||
  ...
  const patchMedia = (which: 'logo' | 'photo', changes: Partial<MediaState>) =>
    setForm((current) =>
      current ? { ...current, [which]: { ...current[which], ...changes } } : current,
    );

  const pickImage = async (which: 'logo' | 'photo', slot: HeroImageVariant, file: File) => {
  ...
  const clearImage = (which: 'logo' | 'photo') => {
    releaseObjectUrl(form[which].preview);
  ...
  const save = async () => {
    ...
    try {
      ...
      const logoFileId = await upload(form.logo, LOGO_ENTITY_TYPE);
      const photoFileId = await upload(form.photo, PHOTO_ENTITY_TYPE);
       ...
       */
      const body: CreatePosOutcomeStoryInput = {
        name: form.name.trim(),
        slug: form.slug.trim(),
        ...(logoFileId ? { logoFileId } : { logoUrl: form.logo.url }),
        ...(photoFileId ? { photoFileId } : { photoUrl: form.photo.url }),
        quote: form.quote.trim(),
        personName: form.personName.trim(),
        personCompany: form.personCompany.trim(),
      ...
      if (isNew) {
        ...
        toast.success('Story updated', 'The public POS page now shows this card.');
        navigate(LIST_PATH);
      }
    } catch (error) {
  ...
  const logoSpec = HERO_IMAGE_SPECS.posOutcomeLogo;
  const photoSpec = HERO_IMAGE_SPECS.posOutcomePhoto;
  ...
  return (
    <>
      <PageHeader
        ...
        description="One network's proof — the photograph behind the card, and the card itself."
        actions={
          <Button
            variant="secondary"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
        <div className="space-y-6">
          <Card>
            <CardHeader
              title="The network"
              subtitle="Who this story is about, and the pictures that carry it."
            />
            <CardBody>
              <FieldGrid>
                <Field
                ...
                >
                  <ImagePicker
                    ...
                    fit="cover"
                    disabled={saving}
                    onPick={(file) => void pickImage('photo', 'posOutcomePhoto', file)}
                ...
                >
                  <ImagePicker
                    ...
                    fit="contain"
                    disabled={saving}
                    onPick={(file) => void pickImage('logo', 'posOutcomeLogo', file)}
                ...
                <Field
                  ...
                  error={errorFor('name')}
                  hint="Also the mark’s alt text, and what the avatar beside the quote draws its initials from."
                >
                  <Input
                    value={form.name}
                ...
                <Field
                  ...
                  error={errorFor('slug')}
                  hint="Identifies the story in links. Renaming the network does not change it."
                >
                  <Input
                    value={form.slug}
          ...
          <Card>
            <CardHeader title="The quote" subtitle="What they said, and who said it." />
            <CardBody>
              <FieldGrid>
                <Field
                  label={RULES.quote.label}
                  error={errorFor('quote')}
                  hint="Typed without quotation marks — the card adds them."
                >
                  <Textarea
                    rows={3}
                    value={form.quote}
                    maxLength={RULES.quote.max}
                    placeholder="Every outlet reports into one place in real time…"
                ...
                <Field
                  label={RULES.personName.label}
                  error={errorFor('personName')}
                  hint="A name or a role — the bold line under the quote."
                >
                  <Input
                ...
                <Field
                  label={RULES.personCompany.label}
                  error={errorFor('personCompany')}
                  hint="The lighter line beneath it. Usually the network’s name."
                >
                  <Input
                    value={form.personCompany}
                    maxLength={RULES.personCompany.max}
                    placeholder="U2 Cake"
                    aria-invalid={!!errorFor('personCompany')}
                    onBlur={() => setTouched((t) => ({ ...t, personCompany: true }))}
                    onChange={(e) => patch({ personCompany: e.target.value })}
                  />
                </Field>
              </FieldGrid>
            </CardBody>
              ...
              <FieldGrid>
                <Field label={RULES.linkLabel.label} error={errorFor('linkLabel')}>
                  <Input
                    ...
                    placeholder="Watch the case study"
          ...
          <Card>
            ...
            <CardBody>
              <FieldGrid cols={1}>
                <Field
                  label="Display order"
                  hint="Lower numbers come first in the rotation. Leave blank to add at the end."
                >
                  <Input
                    type="number"
                    min={0}
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this story? It joins the carousel straight away — you can add its figures next.'
            : 'Are you sure you want to update this story? The public POS page will show it straight away.'
        }
