...
/**
 * The beverage categories - the grid of cards under the copy, four across on
 * desktop.
 *
 * Capped on the server at five full rows. The New button goes dead at the
 * cap. The badge colours cycle by position on the site, so a category has none
 * of its own to edit.
 */

const EDIT_PATH = '/cms/industries/beverage/coverage-section/categories';

/** MAX_BEVERAGE_COVERAGE_CATEGORIES on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = 20;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: BeverageCoverageCategory }
  | { kind: 'status'; record: BeverageCoverageCategory; next: ContentStatus };

export default function CoverageCategoriesCard() {
  const [categories, setCategories] = useState<BeverageCoverageCategory[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The grid of cards, four across on desktop, in order. At most {MAX_STATS}.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          title={
            atLimit
              ? `The grid holds at most ${MAX_STATS} categories`
              : undefined
          }
      ...
      <DataTable<BeverageCoverageCategory>
        data={categories}
        loading={loading}
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first category to take over the section from the site’s built-in set.'
        }
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the category from the grid.'
            : pending?.next === 'ACTIVE'
