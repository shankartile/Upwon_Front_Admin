...
/**
 * The core capabilities - one tab each in the viewer, with its own screenshot.
 *
 * Capped on the server so the tab list stays level with the frame it
 * controls. The New button goes dead at the cap.
 */

const EDIT_PATH = '/cms/industries/beverage/capabilities-section/capabilities';

/** MAX_BEVERAGE_CAPABILITIES on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = 12;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: BeverageCapability }
  | { kind: 'status'; record: BeverageCapability; next: ContentStatus };

export default function CapabilitiesCard() {
  const [cards, setCards] = useState<BeverageCapability[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            One tab each, in order, with its own screenshot. At most {MAX_STATS}.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          title={
            atLimit
              ? `The tab list holds at most ${MAX_STATS} capabilities`
              : undefined
          }
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the capability and its tab.'
            : pending?.next === 'ACTIVE'
              ? 'This capability’s tab will start appearing on the page.'
              : 'This capability’s tab will be removed from the live page but kept here.'
        }
