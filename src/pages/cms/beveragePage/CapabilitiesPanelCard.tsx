...
import type { BeverageCapabilitiesPanel } from '../../../types/beveragePage';
 ...
 * behind it - the same arrangement as the Engineering page's panels. No alt
 * text: the picture is decoration behind the whole section.
 */

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'beverage_capabilities_image';
...
const toForm = (panel: BeverageCapabilitiesPanel): Form => ({
...
export default function CapabilitiesPanelCard() {
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('beverageCapabilitiesBackground', dimensions);
    if (problem) {
  ...
  return (
    <Card className="mt-6">
      <CardHeader
        ...
        subtitle="Covers the whole section, behind the heading, the tabs and the screenshot."
      />
      <CardBody>
        <div className="grid grid-cols-1 items-end gap-6 lg:grid-cols-[320px,1fr]">
          <Field
            label={HERO_IMAGE_SPECS.beverageCapabilitiesBackground.label}
            error={form.imageError ?? undefined}
            hint={HERO_IMAGE_SPECS.beverageCapabilitiesBackground.hint}
          >
            <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          existed
            ? 'Are you sure you want to update the background? The public Beverages & Juices page will show it straight away.'
