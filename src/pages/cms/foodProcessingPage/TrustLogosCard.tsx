...
import { FOOD_PROCESSING_LIMITS, type FoodProcessingTrustLogo } from '../../../types/foodProcessingPage';

/**
 * The customer logos in the trust section - the scrolling marquee under
 * "Trusted by food processing businesses across India".
 *
 * Its own table rather than a field on the section's form: a logo is added the
 * day a brand goes live, which is a different edit from rewriting the
 * heading above it.
 */

const EDIT_PATH = '/cms/industries/food-processing/trust-section/logos';

/** MAX_FOOD_PROCESSING_TRUST_LOGOS on the server. Shown as a hint before the 409 fires. */
const MAX_LOGOS = FOOD_PROCESSING_LIMITS.trustLogos;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: FoodProcessingTrustLogo }
  | { kind: 'status'; record: FoodProcessingTrustLogo; next: ContentStatus };

export default function TrustLogosCard() {
  const [logos, setLogos] = useState<FoodProcessingTrustLogo[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The logo marquee under the heading. At most {MAX_LOGOS}; lower positions come first.
          </p>
        </div>
        <Button
          variant="orange"
      ...
      <DataTable<FoodProcessingTrustLogo>
        data={logos}
        loading={loading}
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first logo — until then this section is hidden on the public page.'
        }
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This removes the logo from the wall and the public page. The record is kept for audit, but no longer appears in this list.'
            : pending?.next === 'ACTIVE'
