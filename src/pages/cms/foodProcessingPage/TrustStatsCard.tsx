...
import { FOOD_PROCESSING_LIMITS, type FoodProcessingTrustStat } from '../../../types/foodProcessingPage';

/**
 * The figures beside the trust photograph - a number and what it counts.
 *
 * Capped at three on the server - the row is three across. The New button goes dead at
 * the cap rather than letting somebody write one the server will refuse.
 */

const EDIT_PATH = '/cms/industries/food-processing/trust-section/stats';

/** MAX_FOOD_PROCESSING_TRUST_STATS on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = FOOD_PROCESSING_LIMITS.trustStats;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: FoodProcessingTrustStat }
  | { kind: 'status'; record: FoodProcessingTrustStat; next: ContentStatus };

export default function TrustStatsCard() {
  const [stats, setStats] = useState<FoodProcessingTrustStat[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The row beside the photograph: a number and what it counts. At most{' '}
            {MAX_STATS} — the row is three across.
          </p>
        </div>
        <Button
          variant="orange"
      ...
      <DataTable<FoodProcessingTrustStat>
        data={stats}
        loading={loading}
