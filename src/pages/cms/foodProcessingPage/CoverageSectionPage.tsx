...
import { FOOD_PROCESSING_LIMITS, type FoodProcessingCoverageItem } from '../../../types/foodProcessingPage';

/**
 * Industry coverage - "Built for a Wide Range of Food Processing Businesses."
 *
 * Two things to edit: the copy over the grid, authored once, and the
 * sub-sector categories themselves - one row per category, searched and paged
 * by the database, with the form on its own page.
 */

const EDIT_PATH = '/cms/industries/food-processing/coverage-section';

/** MAX_FOOD_PROCESSING_COVERAGE_ITEMS on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = FOOD_PROCESSING_LIMITS.coverageItems;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: FoodProcessingCoverageItem }
  | { kind: 'status'; record: FoodProcessingCoverageItem; next: ContentStatus };

export default function FoodProcessingCoverageSectionPage() {
  const [entries, setEntries] = useState<FoodProcessingCoverageItem[]>([]);
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            One card per sub-sector, its illustration above its name. At most {MAX_ENTRIES};
      ...
      <DataTable<FoodProcessingCoverageItem>
        data={entries}
        loading={loading}
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ...
            : pending?.next === 'ACTIVE'
              ? 'This category will start appearing in the Food Processing page grid.'
