...
import { coverageSection as service } from '../../../services/foodProcessingPageService';
...
/**
 * Create / edit one coverage category, as a full page.
 *
 * `:id` of 'new' means create - the same sentinel the other CMS edit screens
 * use. The copy that heads the grid is authored once on the list screen; this
 * form is only the category: its illustration and its name.
 *
 * The image is required: the card is drawn around it, so one without artwork
 * would be a name floating over an empty box.
 */

const LIST_PATH = '/cms/industries/food-processing/coverage-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'food_processing_coverage_image';
 ...
 */
const RULES = {
  label: { label: 'Label', min: 2, max: 120 },
} as const;
...
const toForm = (item: FoodProcessingCoverageItem): Form => ({
  label: item.label,
  status: item.status,
...
export default function FoodProcessingCoverageItemEditPage() {
  ...
  const [item, setItem] = useState<FoodProcessingCoverageItem | null>(null);
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('foodProcessingCoverage', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateFoodProcessingCoverageItemInput = {
      ...
      if (isNew) {
        ...
        toast.success('Category updated', 'The Food Processing page now shows this category.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  const spec = HERO_IMAGE_SPECS.foodProcessingCoverage;
  ...
  return (
    <>
      <PageHeader
        ...
        title={isNew ? 'New coverage category' : 'Edit coverage category'}
        description="One sub-sector in the industry coverage grid on the Food Processing page."
        actions={
          <Button
            variant="secondary"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
        <Card>
          <CardHeader
            title="The category"
            subtitle="The illustration and the name beneath it are all the grid shows."
          />
          <CardBody>
            <FieldGrid>
              <Field
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void save()}
        title={isNew ? 'Create coverage category' : 'Update coverage category'}
        description={
          isNew
            ? 'Are you sure you want to create this category? If it is saved as Active, it will join the grid on the Food Processing page straight away.'
            : 'Are you sure you want to update this category? The Food Processing page will show the change straight away.'
        }
...
function ImagePicker({
  ...
  return (
    <div className="flex items-start gap-4">
      {/* object-contain, matching the card: the artwork is drawn whole. */}
