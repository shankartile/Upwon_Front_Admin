...
import type { FoodProcessingHeroSlide } from '../../../types/foodProcessingPage';

/**
 * Food Processing hero slider admin - the slide list.
 *
...
const EDIT_PATH = '/cms/industries/food-processing/hero-section';

/** MAX_FOOD_PROCESSING_HERO_SLIDES on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = 12;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: FoodProcessingHeroSlide }
  | { kind: 'status'; record: FoodProcessingHeroSlide; next: ContentStatus };

export default function FoodProcessingHeroSectionPage() {
  const [entries, setEntries] = useState<FoodProcessingHeroSlide[]>([]);
  ...
  return (
    <>
      ...
      <DataTable<FoodProcessingHeroSlide>
        data={entries}
        loading={loading}
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ...
            : pending?.next === 'ACTIVE'
              ? 'This slide will start appearing in the Food Processing page slider.'
