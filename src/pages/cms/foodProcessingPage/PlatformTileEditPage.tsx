...
import { platformSection as service } from '../../../services/foodProcessingPageService';
...
const LIST_PATH = '/cms/industries/food-processing/platform-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const ICON_ENTITY_TYPE = 'food_processing_platform_icon';
...
const toForm = (tile: FoodProcessingPlatformTile): Form => ({
  label: tile.label,
  href: tile.href,
  status: tile.status,
...
export default function FoodProcessingPlatformTileEditPage() {
  ...
  const [tile, setTile] = useState<FoodProcessingPlatformTile | null>(null);
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('foodProcessingPlatformIcon', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateFoodProcessingPlatformTileInput = {
      ...
      if (isNew) {
        ...
        toast.success('Tile updated', 'The Food Processing page now shows this tile.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  const spec = HERO_IMAGE_SPECS.foodProcessingPlatformIcon;
  ...
  return (
    <>
      <PageHeader
        ...
        description="One tile in the connected platform grid on the Food Processing page."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this tile? If it is saved as Active, it will join the grid on the Food Processing page straight away.'
            : 'Are you sure you want to update this tile? The Food Processing page will show the change straight away.'
        }
