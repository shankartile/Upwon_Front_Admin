...
import { FOOD_PROCESSING_LIMITS, type FoodProcessingPlatformTile } from '../../../types/foodProcessingPage';

/**
 * The connected platform - "Five Connected Solutions. One Smarter Food Processing
 * Operation."
 *
...
const EDIT_PATH = '/cms/industries/food-processing/platform-section';

/** MAX_FOOD_PROCESSING_PLATFORM_TILES on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = FOOD_PROCESSING_LIMITS.platformTiles;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: FoodProcessingPlatformTile }
  | { kind: 'status'; record: FoodProcessingPlatformTile; next: ContentStatus };

export default function FoodProcessingPlatformSectionPage() {
  const [entries, setEntries] = useState<FoodProcessingPlatformTile[]>([]);
  ...
  return (
    <>
      ...
      <DataTable<FoodProcessingPlatformTile>
        data={entries}
        loading={loading}
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ...
            : pending?.next === 'ACTIVE'
              ? 'This tile will start appearing in the Food Processing page grid.'
