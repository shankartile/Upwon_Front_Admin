...
import type { EngineeringCoveragePanel } from '../../../types/engineeringManufacturingPage';

/**
 * The industry coverage section's background illustration.
 *
 * One record, so it is a form on the tab rather than a list with a form
 * behind it - the same arrangement as the connected platform section's panel.
 * No alt text: the picture is decoration behind the copy.
 */

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'engineering_coverage_image';
...
const toForm = (panel: EngineeringCoveragePanel): Form => ({
...
export default function CoveragePanelCard() {
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('engineeringCoverage', dimensions);
    if (problem) {
  ...
  return (
    <Card className="mt-6">
      <CardHeader
        ...
        subtitle="Feathered into the top right of the section, behind the copy. Desktop only."
      />
      <CardBody>
        <div className="grid grid-cols-1 items-end gap-6 lg:grid-cols-[320px,1fr]">
          <Field
            label={HERO_IMAGE_SPECS.engineeringCoverage.label}
            error={form.imageError ?? undefined}
            hint={HERO_IMAGE_SPECS.engineeringCoverage.hint}
          >
            <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          existed
            ? 'Are you sure you want to update the background? The public Engineering & Manufacturing page will show it straight away.'
