...
/**
 * Create / edit one business type in the industry coverage grid, as a full
 * page.
 *
...
const LIST_PATH = '/cms/industries/engineering-manufacturing/coverage-section';

/** Field rules, mirroring the server-side coverage section validator. */
const RULES = {
  label: { label: 'Business type', min: 2, max: 120, required: true },
} as const;
...
const toForm = (category: EngineeringCoverageCategory): Form => ({
...
export default function EngineeringCoverageCategoryEditPage() {
  ...
  const [category, setCategory] = useState<EngineeringCoverageCategory | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="Business type" description="Could not load this business type." />
        <Card>
          <CardBody>
  ...
  const save = async () => {
    setSaving(true);
    try {
      const body: CreateEngineeringCoverageCategoryInput = {
      ...
      toast.error('Could not save business type', errorMessage(error));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      <PageHeader
        ...
        title={isNew ? 'New business type' : 'Edit business type'}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
        <div className="space-y-6">
          <Card>
            <CardHeader title="The card" subtitle="A short name for the business type." />
            <CardBody className="space-y-4">
              ...
              {/* The card as the grid draws it: orange icon on a round wash. */}
              <div className="flex max-w-xs items-center gap-4 rounded-[14px] border border-cream-300 bg-white p-5 dark:border-navy-800 dark:bg-navy-950/50">
                <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-[#fdf0e6] text-orange-500">
                  <IconGlyph name={form.icon} className="h-6 w-6" />
                </span>
                <span className="min-w-0 text-[14px] font-semibold leading-snug text-charcoal dark:text-cream-100">
                  {form.label.trim() || 'Business type'}
                </span>
              </div>
            </CardBody>
          </Card>
          ...
          <Card>
            <CardHeader title="Icon" subtitle="Drawn in the round wash beside the label." />
            <CardBody>
              <IconPicker
                value={form.icon}
                options={icons}
      ...
      <div className="sticky bottom-0 z-10 -mx-4 -mb-4 mt-6 border-t hairline bg-cream-50/95 px-4 py-3 backdrop-blur sm:-mx-6 sm:-mb-6 sm:px-6 dark:bg-navy-900/95">
        <div className="flex items-center justify-end gap-3">
          ...
          >
            {isNew ? 'Create business type' : 'Save changes'}
          </Button>
        </div>
      </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void save()}
        title={isNew ? 'Create business type' : 'Update business type'}
        description={
          isNew
            ? 'Are you sure you want to create this business type? It will appear in the grid straight away.'
            : 'Are you sure you want to update this business type? The public Engineering & Manufacturing page will show it straight away.'
        }
