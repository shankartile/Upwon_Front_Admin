...
/**
 * The core capabilities - one per card drawn into the section's artwork.
 *
 * Capped at eight on the server, because that is how many cards the artwork
 * draws. The New button goes dead at the cap rather than letting somebody
 * write a ninth that would have nowhere to go. Display order decides which
 * card a capability fills and the number printed on it.
 */

const EDIT_PATH = '/cms/industries/engineering-manufacturing/capabilities-section';

/** MAX_ENGINEERING_CAPABILITIES on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = 8;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: EngineeringCapability }
  | { kind: 'status'; record: EngineeringCapability; next: ContentStatus };

export default function CapabilitiesCard() {
  const [capabilities, setCapabilities] = useState<EngineeringCapability[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            One per card on the artwork, in order — the first fills card 01. The artwork draws{' '}
            {MAX_STATS} cards, so keep all {MAX_STATS} active for the desktop layout to stay full.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          title={
            atLimit
              ? `The artwork holds at most ${MAX_STATS} capabilities`
              : undefined
          }
      ...
      <DataTable<EngineeringCapability>
        data={capabilities}
        loading={loading}
        ...
        columns={[
          {
            ...
            width: '76px',
            render: (row) => (
              // Zero-padded, the way the artwork prints it.
          ...
          },
          {
            ...
            width: '72px',
            render: (row) => (
              // Tinted with the capability's own colours, so the list reads like
              // the stacked cards rather than a column of grey glyphs.
              <span
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the capability from the section.'
            : pending?.next === 'ACTIVE'
              ? 'This capability will start appearing in the section.'
              : 'This capability will be removed from the live section but kept here — its card on the artwork is left empty.'
        }
