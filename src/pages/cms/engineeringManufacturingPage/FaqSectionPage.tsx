...
import { faqSection as faqSectionService } from '../../../services/engineeringManufacturingPageService';
import { DEFAULT_PAGE_SIZE } from '../../../config/constants';
import { errorMessage } from '../../../lib/http';
import { SectionCopyCard } from '../homePage/SectionCopyCard';
import { fmtDate, relativeTime } from '../../../lib/formatters';
import { STATUS_LABELS, type ContentStatus } from '../../../types/homePage';
import type { EngineeringFaqEntry as FaqEntry } from '../../../types/engineeringManufacturingPage';

/**
 * Engineering & Manufacturing FAQ admin - the question list.
 *
 * The same screen as the other home page sections: one row per question,
 * searched and paged by the database, with the form on its own page.
 */

const EDIT_PATH = '/cms/industries/engineering-manufacturing/faq-section';

/** MAX_ENGINEERING_FAQ_ENTRIES on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = 24;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: FaqEntry }
  | { kind: 'status'; record: FaqEntry; next: ContentStatus };

export default function EngineeringFaqSectionPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { rows, meta } = await faqSectionService.list({
  ...
  const runPending = async () => {
    ...
    try {
      if (pending.kind === 'delete') {
        await faqSectionService.remove(pending.record.id);
        toast.success('Question deleted');
      } else {
        await faqSectionService.setStatus(pending.record.id, pending.next);
        toast.success(pending.next === 'ACTIVE' ? 'Question activated' : 'Question deactivated');
      }
      await load();
    } catch (error) {
    ...
    <>
      ...
      <div className="mb-4 flex items-center justify-end gap-3">
        <Button
          ...
          disabled={atLimit}
          title={atLimit ? `The section holds at most ${MAX_ENTRIES} questions` : undefined}
      ...
      <DataTable<FaqEntry>
        data={entries}
        loading={loading}
        emptyTitle={isNarrowed ? 'No matching questions' : 'No questions yet'}
        emptyDescription={
          isNarrowed
            ? 'Try a different search term, or clear the status filter.'
            : 'Add the first question to take over this section from the site’s built-in list.'
        }
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the question and its answer from the section.'
            : pending?.next === 'ACTIVE'
              ? 'This question will start appearing in the Engineering & Manufacturing page FAQ.'
              : 'This question will be removed from the live FAQ but kept here.'
        }
