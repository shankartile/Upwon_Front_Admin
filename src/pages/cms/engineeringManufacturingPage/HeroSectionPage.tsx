...
import type { EngineeringHeroSlide } from '../../../types/engineeringManufacturingPage';

/**
 * Engineering hero slider admin - the slide list.
 *
 * No section copy card here, unlike the page's other sections: each slide
 * carries its own eyebrow, headline and subhead, because the slider shows four
 * different pitches.
 */

const EDIT_PATH = '/cms/industries/engineering-manufacturing/hero-section';

/** MAX_ENGINEERING_HERO_SLIDES on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = 12;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: EngineeringHeroSlide }
  | { kind: 'status'; record: EngineeringHeroSlide; next: ContentStatus };

export default function EngineeringHeroSectionPage() {
  const [entries, setEntries] = useState<EngineeringHeroSlide[]>([]);
  ...
  return (
    <>
      ...
      <DataTable<EngineeringHeroSlide>
        data={entries}
        loading={loading}
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first slide to take over the slider from the site’s built-in set.'
        }
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the slide, along with its copy and buttons, from the slider.'
            : pending?.next === 'ACTIVE'
              ? 'This slide will start appearing in the Engineering & Manufacturing page slider.'
