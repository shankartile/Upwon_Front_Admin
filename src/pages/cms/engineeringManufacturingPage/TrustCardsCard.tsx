...
/**
 * The figure cards - the row of three above the client marquee.
 *
 * Capped at three on the server, because that is what the row holds. The New
 * button goes dead at the cap rather than letting somebody write a fourth card
 * that would wrap onto a row of its own.
 */

const EDIT_PATH = '/cms/industries/engineering-manufacturing/trust-section/cards';

/** MAX_ENGINEERING_TRUST_CARDS on the server. Shown as a hint before the 409 fires. */
const MAX_STATS = 3;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: EngineeringTrustCard }
  | { kind: 'status'; record: EngineeringTrustCard; next: ContentStatus };

export default function TrustCardsCard() {
  const [cards, setCards] = useState<EngineeringTrustCard[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The row of cards above the marquee. At most {MAX_STATS} — that is what it holds.
            Each card can turn over between two figures.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          title={
            atLimit
              ? `The row holds at most ${MAX_STATS} cards`
              : undefined
          }
      ...
      <DataTable<EngineeringTrustCard>
        data={cards}
        loading={loading}
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first card to take over the row from the site’s built-in set.'
        }
        ...
        columns={[
          ...
          },
          {
            ...
            render: (row) => (
              ...
              // row rather than a column of grey glyphs.
              <span
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the card from the row.'
            : pending?.next === 'ACTIVE'
              ? 'This card will start appearing in the row.'
