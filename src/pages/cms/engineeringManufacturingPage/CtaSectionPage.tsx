...
import { ctaSection as service } from '../../../services/engineeringManufacturingPageService';
...
/**
 * The Engineering & Manufacturing page's closing band.
 *
 * One record, so this is the tab itself rather than a list with a form behind
 * it: the copy card at the top, the artwork pair, and the buttons - the POS
 * band's form without its footnote and handwritten note.
 *
 * Both crops are of the same picture. The desktop one keeps an empty panel on
 * its left that the copy is laid over; the phone one is a banner above the
 * copy.
 */

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'engineering_cta_image';

/**
 * Field rules, mirroring the server-side Engineering closing band validator.
 *
...
const toForm = (section: EngineeringCtaSection): Form => ({
...
export default function EngineeringCtaSectionPage() {
  ...
  const pickImage = async (which: 'desktop' | 'mobile', file: File) => {
    const slot: HeroImageVariant = which === 'desktop' ? 'engineeringCtaDesktop' : 'engineeringCtaMobile';
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: UpsertEngineeringCtaSectionInput = {
        ...(desktopImageFileId
      ...
      toast.success('Closing band saved', 'The public Engineering & Manufacturing page now shows this content.');
    } catch (error) {
    ...
    <>
      <SectionCopyCard
        ...
        placeholders={{
          ...
          subtext:
            'Connect procurement, raw materials, bill of materials, production planning, quality, inventory, warehouses, sales, dispatch, people, and business operations through one platform…',
        }}
      />
          ...
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                ...
                hint={HERO_IMAGE_SPECS.engineeringCtaDesktop.hint}
              >
                <ArtworkPicker
                  aspect="aspect-[1600/599]"
              ...
              <Field
                ...
                hint={HERO_IMAGE_SPECS.engineeringCtaMobile.hint}
              >
                <ArtworkPicker
                  aspect="aspect-[440/956]"
      ...
      <ConfirmDialog
        ...
        description={
          existed
            ? 'Are you sure you want to update this band? The public Engineering & Manufacturing page will show the new content straight away.'
