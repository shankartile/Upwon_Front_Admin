...
import type { CreateWhyUpwonClientLogoInput, WhyUpwonClientLogo } from '../../../types/whyUpwonPage';

/**
 * Create / edit one client logo, as a full page.
 *
 * `:id` of 'new' means create - the same sentinel the other CMS edit screens
 * use.
 *
 * Unlike the optional artwork elsewhere, the image here is required: the row
 * exists only to put a mark in the client wall, so one without an image would be a
...
const LOGO_ENTITY_TYPE = 'why_upwon_client_logo';
 ...
 * optional: this client wall has no other text, so the name is the only thing a
...
const toForm = (logo: WhyUpwonClientLogo): Form => ({
  alt: logo.alt,
  status: logo.status,
...
export default function WhyUpwonClientLogoEditPage() {
  ...
  const [logo, setLogo] = useState<WhyUpwonClientLogo | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="Client logo" description="Could not load this logo." />
        <Card>
          <CardBody>
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateWhyUpwonClientLogoInput = {
      ...
      if (isNew) {
        ...
        toast.success('Logo updated', 'The public Why UpWon page now shows this client wall.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        title={isNew ? 'New client logo' : 'Edit client logo'}
        description="One logo in the scrolling client wall."
        actions={
          <Button
            variant="secondary"
          ...
          <Card>
            ...
            <CardBody>
              <FieldGrid cols={1}>
                ...
                <Field
                  label="Status"
                  hint="Inactive keeps the logo here but removes it from the live client wall."
                >
                  <Select
                    value={form.status}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void save()}
        title={isNew ? 'Create client logo' : 'Update client logo'}
        description={
          isNew
            ? 'Are you sure you want to create this logo? It will join the client wall straight away.'
            : 'Are you sure you want to update this logo? The public Why UpWon page will show it straight away.'
        }
...
function LogoPicker({
  ...
  return (
    <div className="flex items-start gap-4">
      {/* object-contain, matching the client wall: logos are never cropped. */}
