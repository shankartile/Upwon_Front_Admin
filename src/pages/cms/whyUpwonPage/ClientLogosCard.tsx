...
import type { WhyUpwonClientLogo } from '../../../types/whyUpwonPage';

/**
 * The client logos - the scrolling wall at the foot of the section.
 *
 * Its own table rather than a field on the section's copy: a logo is added the
 * day a brand goes live, which is a different edit from rewriting the heading.
 */

const EDIT_PATH = '/cms/why-upwon/testimonials-section/logos';

/** MAX_WHY_UPWON_CLIENT_LOGOS on the server. Shown as a hint before the 409 fires. */
const MAX_LOGOS = 24;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: WhyUpwonClientLogo }
  | { kind: 'status'; record: WhyUpwonClientLogo; next: ContentStatus };

export default function ClientLogosCard() {
  const [logos, setLogos] = useState<WhyUpwonClientLogo[]>([]);
  ...
  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The wall of logos at the foot of the section. It scrolls, so the order is the order it starts in.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          disabled={atLimit}
          title={atLimit ? `The client wall holds at most ${MAX_LOGOS} logos` : undefined}
      ...
      <DataTable<WhyUpwonClientLogo>
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first logo to take over the client wall from the site’s built-in set.'
        }
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the logo from the client wall.'
            : pending?.next === 'ACTIVE'
              ? 'This logo will start appearing in the client wall.'
              : 'This logo will be removed from the live client wall but kept here.'
        }
