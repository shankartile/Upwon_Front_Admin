...
/** The shipped hero, so a first run starts on what the page already shows. */
const EMPTY: Form = {
  imageAlt: '',
...
const toForm = (section: WhyUpwonHeroSection): Form => ({
...
export default function WhyUpwonHeroSectionPage() {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: UpsertWhyUpwonHeroSectionInput = {
        ...(desktopImageFileId
      ...
      toast.success('Hero saved', 'The public Why UpWon page now shows this content.');
    } catch (error) {
      toast.error('Could not save the hero', errorMessage(error));
    } finally {
      setSaving(false);
    }
  };
    ...
    <>
      <SectionCopyCard
        ...
        placeholders={{
          ...
          subtext:
            'Manufacturing businesses need more than disconnected software and manual processes…',
        }}
      />
      ...
      <div className="sticky bottom-0 z-10 -mx-4 -mb-4 mt-6 border-t hairline bg-cream-50/95 px-4 py-3 backdrop-blur sm:-mx-6 sm:-mb-6 sm:px-6 dark:bg-navy-900/95">
        <div className="flex items-center justify-end gap-3">
          ...
          >
            {existed ? 'Save changes' : 'Create hero'}
          </Button>
        </div>
      </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void save()}
        title={existed ? 'Update the hero' : 'Create the hero'}
        description={
          existed
            ? 'Are you sure you want to update the hero? The public Why UpWon page will show the new content straight away.'
            : 'Are you sure you want to create the hero? It replaces the one the site ships with straight away.'
        }
