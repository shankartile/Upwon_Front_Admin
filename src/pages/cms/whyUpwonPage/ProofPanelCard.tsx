...
/**
 * The product proof artwork: the dashboard the callouts are pinned to, and
 * what it shows.
 *
...
const IMAGE_ENTITY_TYPE = 'why_upwon_proof_panel';
...
const toForm = (panel: WhyUpwonProofPanel): Form => ({
...
export default function ProofPanelCard() {
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('whyUpwonProofArtwork', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: UpsertWhyUpwonProofPanelInput = {
        ...(imageFileId
      ...
      toast.success('Artwork saved', 'The public Why UpWon page now shows it.');
    } catch (error) {
  ...
  return (
    <Card className="mt-6">
      <CardHeader
        title="Artwork"
        subtitle="The dashboard the callouts are pinned to, and what it shows."
      />
      <CardBody>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[320px,1fr]">
          <Field
            label={HERO_IMAGE_SPECS.whyUpwonProofArtwork.label}
            error={form.imageError ?? undefined}
            hint={HERO_IMAGE_SPECS.whyUpwonProofArtwork.hint}
          >
            <ImagePicker
          ...
          <div className="space-y-4">
            ...
            >
              <Textarea
                ...
                placeholder="The UpWon dashboard showing sales, purchase, customer and product figures…"
...
function ImagePicker({
  ...
  return (
    <div className="space-y-2">
      {/* 3:2 and object-contain, the shape the live section draws it at. */}
