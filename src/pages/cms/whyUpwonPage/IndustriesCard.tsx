...
/**
 * The industry trust cards - a photo, a name and a link each, in the row under
 * the heading.
 *
 * Capped on the server. The New button goes dead at the cap.
 */

const EDIT_PATH = '/cms/why-upwon/industries-section/industries';

/** MAX_WHY_UPWON_INDUSTRIES on the server. Shown as a hint before the 409 fires. */
const MAX_CATEGORIES = 21;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: WhyUpwonIndustry }
  | { kind: 'status'; record: WhyUpwonIndustry; next: ContentStatus };

export default function IndustriesCard() {
  const [industries, setIndustries] = useState<WhyUpwonIndustry[]>([]);
    ...
    <section className="mt-8">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The cards in the row under the heading, in order. At most {MAX_CATEGORIES}.
          </p>
        </div>
        <Button
          variant="orange"
          ...
          disabled={atLimit}
          title={atLimit ? `The row holds at most ${MAX_CATEGORIES} industries` : undefined}
      ...
      {loadError && (
        <div className="mb-4 rounded-xl border border-orange-200 bg-orange-50 p-4 text-sm dark:border-orange-900/40 dark:bg-orange-900/10">
          <p className="font-medium text-orange-800 dark:text-orange-300">Could not load industries</p>
      ...
      <DataTable<WhyUpwonIndustry>
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first industry to take over the row from the site’s built-in set.'
        }
          ...
          },
          {
            ...
            render: (row) => (
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-charcoal-light dark:text-navy-300">{row.href}</p>
              </div>
            ),
          },
          {
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the industry and its card.'
            : pending?.next === 'ACTIVE'
              ? 'This industry’s card will start appearing in the row.'
              : 'This industry’s card will be removed from the live row but kept here.'
        }
