...
/**
 * Platform Integrations admin - the logo list.
 *
 * The same screen as the other home page sections: one row per logo, searched
 * and paged by the database, with the form on its own page.
 */

const EDIT_PATH = '/cms/home-page/integrations-section';

/** MAX_INTEGRATIONS_ENTRIES on the server. Shown as a hint before the 409. */
const MAX_ENTRIES = 24;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: IntegrationsEntry }
  | { kind: 'status'; record: IntegrationsEntry; next: ContentStatus };

export default function IntegrationsSectionPage() {
  const [entries, setEntries] = useState<IntegrationsEntry[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { rows, meta } = await integrationsSectionService.list({
  ...
  const runPending = async () => {
    ...
    try {
      if (pending.kind === 'delete') {
        await integrationsSectionService.remove(pending.record.id);
        toast.success('Logo deleted');
      } else {
        await integrationsSectionService.setStatus(pending.record.id, pending.next);
    ...
    <>
      <SectionCopyCard
        ...
        placeholders={{
          ...
          "subtext": "UPWON seamlessly integrates with the systems you already use…"
        }}
      />
      ...
      <div className="mb-4 flex items-center justify-end gap-3">
        <Button
          ...
          disabled={atLimit}
          title={atLimit ? `The section holds at most ${MAX_ENTRIES} logos` : undefined}
      ...
      <DataTable<IntegrationsEntry>
        ...
        emptyDescription={
          isNarrowed
            ...
            : 'Add the first logo to take over this section from the site’s built-in sphere.'
        }
        ...
        columns={[
          ...
          },
          {
            ...
            render: (row) =>
              row.logo ? (
                /*
                 * object-contain on a light tile, the way the badge renders on
                 * the sphere: these marks run from 2:1 to nearly 8:1, so
                 * cropping them to a fixed box would misrepresent every one.
                 */
                <span className="flex h-12 w-16 items-center justify-center rounded-md border border-cream-300 bg-white p-1 dark:border-navy-800">
                  <img
            ...
            render: (row) => (
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-charcoal-light dark:text-navy-300">
                  {row.logoFileId ? 'Uploaded' : 'From the site’s images'}
                </p>
              </div>
            ),
          },
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ? 'This permanently removes the logo from the section.'
            : pending?.next === 'ACTIVE'
              ? 'This logo will start appearing on the home page integration sphere.'
