...
import type { HeroImageVariant } from '../../../lib/heroImageSpec';
import {
  CLEARED_IMAGE_SLOT,
  EMPTY_IMAGE_SLOT,
  IMAGE_URL_MAX,
...
const VARIANTS: readonly HeroImageVariant[] = ['desktop', 'mobile'];
 ...
 */
const SLOT_FIELDS: Record<HeroImageVariant, string[]> = {
 ...
 */
const PREVIEW_BOX: Record<HeroImageVariant, string> = {
  desktop: 'h-28 w-48',
...
type Touched = Partial<Record<TextFieldName | `${HeroImageVariant}Url`, boolean>>;
...
export default function HeroSlideEditPage({
  ...
  const urlErrors = useMemo(
    (): Record<HeroImageVariant, string | null> => ({
  ...
  const urlErrorFor = (variant: HeroImageVariant): string | undefined =>
   ...
   */
  const slotErrorFor = (variant: HeroImageVariant): string | undefined =>
    urlErrorFor(variant) ??
  ...
  const setSlot = (variant: HeroImageVariant, slot: ImageSlot) => {
  ...
  const clearImage = (variant: HeroImageVariant) => setSlot(variant, { ...CLEARED_IMAGE_SLOT });
