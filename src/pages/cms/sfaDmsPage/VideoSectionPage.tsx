...
/**
 * Video Section admin - the video list.
 *
 * The same screen as the page's other sections, against the same shape of
 * data: one row per video, searched and paged by the database, with the form
 * on its own page.
 *
...
const EDIT_PATH = '/cms/products/sfa-dms/video-section';
...
type Pending =
  | { kind: 'delete'; record: SfaVideoEntry }
  | { kind: 'status'; record: SfaVideoEntry; next: ContentStatus };

export default function SfaVideoSectionPage() {
  const [entries, setEntries] = useState<SfaVideoEntry[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { rows, meta } = await videoSectionService.list({
       ...
       */
      const live = await videoSectionService.list({ status: 'ACTIVE', limit: 1 });
  ...
  const runPending = async () => {
    ...
    try {
      if (pending.kind === 'delete') {
        await videoSectionService.remove(pending.record.id);
        toast.success('Entry deleted');
      } else {
        await videoSectionService.setStatus(pending.record.id, pending.next);
