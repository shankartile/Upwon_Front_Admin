...
import { heroSection } from '../../../services/sfaDmsPageService';
...
import type { CreateSfaHeroSlideInput, SfaHeroSlide } from '../../../types/sfaDmsPage';

/**
 * Create / edit one SFA-DMS hero slide, as a full page.
 *
...
const LIST_PATH = '/cms/products/sfa-dms/hero-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'sfa_hero_slide';
...
const toForm = (slide: SfaHeroSlide): Form => ({
...
export default function SfaHeroSlideEditPage() {
  ...
  const [slide, setSlide] = useState<SfaHeroSlide | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="SFA-DMS hero slide" description="Could not load this slide." />
        <Card>
          <CardBody>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('sfaHero', dimensions);
    if (problem) {
  ...
  const pickMobileImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('sfaHeroMobile', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreateSfaHeroSlideInput = {
      ...
      if (isNew) {
        ...
        toast.success('Slide updated', 'The public SFA-DMS page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One slide of the SFA-DMS page slider."
        actions={
          <Button
            variant="secondary"
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                label={HERO_IMAGE_SPECS.sfaHero.label}
                error={form.imageError ?? undefined}
                hint={HERO_IMAGE_SPECS.sfaHero.hint}
              >
                <ImagePicker
              ...
              <Field
                label={HERO_IMAGE_SPECS.sfaHeroMobile.label}
                error={form.mobileImageError ?? undefined}
                hint={HERO_IMAGE_SPECS.sfaHeroMobile.hint}
              >
                <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this slide? The public SFA-DMS page will show the new content straight away.'
        }
