...
const VIDEO_ENTITY_TYPE = 'sfa_video';
...
const toForm = (entry: SfaVideoEntry): Form => ({
  status: entry.status,
...
export default function SfaVideoEntryEditPage() {
  ...
  useEffect(() => {
    if (!isNew) return;
    let cancelled = false;
    videoSectionService.list({ status: 'ACTIVE', limit: 1 }).then(({ rows }) => {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreateSfaVideoEntryInput = {
        status: form.status,
        /*
      ...
      if (isNew) {
        await videoSectionService.create(body);
        toast.success('Entry created');
      } else {
        await videoSectionService.update(id!, body);
        toast.success('Entry updated', 'The public SFA-DMS page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="The product video under “Built for Distribution”. One video is live at a time."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this entry? If it is the first active one, the SFA-DMS page will show it straight away.'
            : 'Are you sure you want to update this entry? The public SFA-DMS page will show the new content straight away.'
        }
