...
import type { CreateSfaProofStatInput, SfaProofStat } from '../../../types/sfaDmsPage';
 ...
 *
 * The figure's minimum is one character, not two: "8" is a perfectly good
 * number on its own.
 */
const RULES = {
...
const EMPTY: Form = { value: '', label: '', displayOrder: '', status: 'ACTIVE' };

const toForm = (stat: SfaProofStat): Form => ({
  value: stat.value,
  label: stat.label,
...
export default function SfaProofStatEditPage() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const body: CreateSfaProofStatInput = {
      ...
      if (isNew) {
        ...
        toast.success('Figure updated', 'The public SFA-DMS page now shows this number.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One tile of the two-by-two grid beside the proof card."
        actions={
          <Button
            variant="secondary"
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,360px]">
        <Card>
          <CardHeader
            title="The number"
            subtitle="Rendered verbatim, so the separator, the percent sign and the plus are yours."
          />
          <CardBody className="space-y-4">
            <FieldGrid>
              ...
              <Field
                ...
                hint={`The line under the figure. ${form.label.trim().length}/${RULES.label.max}`}
              >
                <Input
                  value={form.label}
                  maxLength={RULES.label.max}
                  placeholder="Distribution users"
            ...
            {(form.value.trim() || form.label.trim()) && (
              // Roughly the live tile, so a long label's wrapping is visible
              // here rather than on the published page.
              <div className="mx-auto w-full max-w-xs rounded-3xl border border-cream-300 bg-white p-6 text-center dark:border-navy-800 dark:bg-navy-950/50">
                <p className="text-4xl font-semibold tracking-tight text-charcoal dark:text-cream-100">
                  {form.value.trim() || '—'}
                </p>
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this figure? The public SFA-DMS page will show it straight away.'
        }
