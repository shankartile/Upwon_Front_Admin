...
import type { SfaProofLogo } from '../../../types/sfaDmsPage';

/**
 * The customer logos inside the proof card - the marquee that scrolls under
 * "Trusted by".
 *
...
const EDIT_PATH = '/cms/products/sfa-dms/proof-section/logos';

/** MAX_SFA_PROOF_LOGOS on the server. Shown as a hint before the 409 fires. */
const MAX_LOGOS = 24;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: SfaProofLogo }
  | { kind: 'status'; record: SfaProofLogo; next: ContentStatus };

export default function ProofLogosCard() {
  const [logos, setLogos] = useState<SfaProofLogo[]>([]);
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-charcoal-light dark:text-navy-300">
            The marquee inside the card. It scrolls, so the order is the order it starts in.
          </p>
        </div>
        <Button
          variant="orange"
