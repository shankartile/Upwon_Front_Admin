...
import type { CreateSfaProofLogoInput, SfaProofLogo } from '../../../types/sfaDmsPage';
...
const LOGO_ENTITY_TYPE = 'sfa_proof_logo';
...
const toForm = (logo: SfaProofLogo): Form => ({
  alt: logo.alt,
  status: logo.status,
...
export default function SfaProofLogoEditPage() {
  ...
  const [logo, setLogo] = useState<SfaProofLogo | null>(null);
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateSfaProofLogoInput = {
      ...
      if (isNew) {
        ...
        toast.success('Logo updated', 'The public SFA-DMS page now shows this marquee.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One mark in the marquee inside the proof card."
        actions={
          <Button
            variant="secondary"
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this logo? The public SFA-DMS page will show it straight away.'
        }
