...
/**
 * Create / edit one integration logo, as a full page.
 *
 * `:id` of 'new' means create - the same sentinel the other CMS edit screens
 * use.
 *
 * The image is required: the row contributes nothing else, so one without a
 * mark is a gap on the sphere rather than a variation.
 */
...
const LOGO_ENTITY_TYPE = 'fms_integration_logo';

/**
 * Field rules, mirroring the server-side integrations validator.
 *
 * The brand name is required: the mark carries no text of its own, so this is
 * the only thing a screen reader can read in its place.
 */
const RULES = {
  logoAlt: { label: 'Brand name', min: 1, max: 160, required: true },
} as const;
...
const toForm = (logo: FmsIntegrationLogo): Form => ({
  logoAlt: logo.logoAlt,
  status: logo.status,
  displayOrder: String(logo.displayOrder),
  fileId: logo.logoFileId,
  logoUrl: logo.logoUrl,
  file: null,
  preview: assetUrl(logo.logo) ?? null,
  imageError: null,
});
...
export default function FmsIntegrationLogoEditPage() {
  ...
  const errors = useMemo(() => {
    ...
    return { logoAlt: validateField('logoAlt', form.logoAlt) };
  }, [form]);

  /** The rule the table also enforces: a logo row has to carry an image. */
  const imageProblem = useMemo(() => {
    if (!form) return null;
    const hasImage = Boolean(form.file || form.fileId || form.logoUrl);
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('integrationsLogo', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateFmsIntegrationLogoInput = {
        ...(logoFileId ? { logoFileId } : { logoUrl: form.logoUrl }),
      ...
      if (isNew) {
        ...
        toast.success('Logo updated', 'The public FMS page now shows this sphere.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  const spec = HERO_IMAGE_SPECS.integrationsLogo;
  ...
  return (
    <>
      <PageHeader
        ...
        description="One mark pinned around the integration sphere."
        actions={
          <Button
            variant="secondary"
          ...
          <Card>
            ...
            <CardBody>
              <FieldGrid cols={1}>
                <Field
                  label="Display order"
                  hint="Lower numbers come first, which is where the mark sits on the sphere. Leave blank to add at the end."
                >
                  <Input
                    type="number"
                    min={0}
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ? 'Are you sure you want to create this logo? It joins the sphere straight away.'
            : 'Are you sure you want to update this logo? The public FMS page will show it straight away.'
        }
