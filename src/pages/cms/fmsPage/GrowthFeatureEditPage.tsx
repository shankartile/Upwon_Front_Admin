...
import type { CreateFmsGrowthFeatureInput, FmsGrowthFeature } from '../../../types/fmsPage';
...
const toForm = (feature: FmsGrowthFeature): Form => ({
  label: feature.label,
...
export default function FmsGrowthFeatureEditPage() {
  const { tierId, id } = useParams<{ tierId: string; id: string }>();
  const isNew = id === 'new';
  const navigate = useNavigate();
  const toast = useToast();

  const backPath = `/cms/products/fms/growth-section/tiers/${tierId}`;

  const [form, setForm] = useState<Form | null>(isNew ? { ...EMPTY } : null);
  const [feature, setFeature] = useState<FmsGrowthFeature | null>(null);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const body: CreateFmsGrowthFeatureInput = {
      ...
      if (isNew) {
        ...
        toast.success('Tick updated', 'The public FMS page now shows this card.');
      }
      navigate(backPath);
    } catch (error) {
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this tick? The public FMS page will show it straight away.'
        }
