...
import type { FmsGrowthFeature } from '../../../types/fmsPage';
...
/** MAX_FMS_GROWTH_FEATURES on the server. Shown before the 409 fires. */
const MAX_FEATURES = 12;

const BASE = '/cms/products/fms/growth-section/tiers';

type Pending =
  | { kind: 'delete'; record: FmsGrowthFeature }
  | { kind: 'status'; record: FmsGrowthFeature; next: ContentStatus };

export default function GrowthFeaturesCard({ tierId }: { tierId: string }) {
  const editPath = `${BASE}/${tierId}/features`;

  const [features, setFeatures] = useState<FmsGrowthFeature[]>([]);
