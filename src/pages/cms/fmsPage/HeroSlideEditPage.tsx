...
import { heroSection } from '../../../services/fmsPageService';
...
import type { CreateFmsHeroSlideInput, FmsHeroSlide } from '../../../types/fmsPage';

/**
 * Create / edit one FMS hero slide, as a full page.
 *
...
const LIST_PATH = '/cms/products/fms/hero-section';

/** The entity type these uploads are tagged with, to make them publicly servable. */
const IMAGE_ENTITY_TYPE = 'fms_hero_slide';
...
const toForm = (slide: FmsHeroSlide): Form => ({
...
export default function FmsHeroSlideEditPage() {
  ...
  const [slide, setSlide] = useState<FmsHeroSlide | null>(null);
  ...
  if (loadError) {
    return (
      <>
        <PageHeader title="FMS hero slide" description="Could not load this slide." />
        <Card>
          <CardBody>
  ...
  const pickImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('fmsHero', dimensions);
    if (problem) {
  ...
  const pickMobileImage = async (file: File) => {
    ...
    const problem = checkHeroImageDimensions('fmsHeroMobile', dimensions);
    if (problem) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const body: CreateFmsHeroSlideInput = {
      ...
      if (isNew) {
        ...
        toast.success('Slide updated', 'The public FMS page now shows this content.');
      }
      navigate(LIST_PATH);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="One slide of the FMS page slider."
        actions={
          <Button
            variant="secondary"
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardBody className="space-y-5">
              <Field
                label={HERO_IMAGE_SPECS.fmsHero.label}
                error={form.imageError ?? undefined}
                hint={HERO_IMAGE_SPECS.fmsHero.hint}
              >
                <ImagePicker
              ...
              <Field
                label={HERO_IMAGE_SPECS.fmsHeroMobile.label}
                error={form.mobileImageError ?? undefined}
                hint={HERO_IMAGE_SPECS.fmsHeroMobile.hint}
              >
                <ImagePicker
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this slide? The public FMS page will show the new content straight away.'
        }
