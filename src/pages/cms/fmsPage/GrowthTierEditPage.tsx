...
import type { CreateFmsGrowthTierInput, FmsGrowthTier } from '../../../types/fmsPage';
...
const LIST_PATH = '/cms/products/fms/growth-section';
...
const toForm = (tier: FmsGrowthTier): Form => ({
  name: tier.name,
  slug: tier.slug,
  lead: tier.lead,
...
function validateField(name: TextFieldName, raw: string): string | null {
  ...
  // Mirrors fms_growth_tiers_slug_format_check.
...
export default function FmsGrowthTierEditPage() {
  ...
  const [tier, setTier] = useState<FmsGrowthTier | null>(null);
  ...
  const save = async () => {
    setSaving(true);
    try {
      const body: CreateFmsGrowthTierInput = {
        name: form.name.trim(),
        slug: form.slug.trim(),
        lead: form.lead.trim(),
      ...
      if (isNew) {
        ...
        toast.success('Tier updated', 'The public FMS page now shows this card.');
        navigate(LIST_PATH);
      }
    } catch (error) {
      ...
      <ConfirmDialog
        ...
        description={
          isNew
            ...
            : 'Are you sure you want to update this tier? The public FMS page will show it straight away.'
        }
