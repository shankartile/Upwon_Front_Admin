...
import type { CreateFmsProofLogoInput, FmsProofLogo } from '../../../types/fmsPage';
...
const LOGO_ENTITY_TYPE = 'fms_proof_logo';
...
const toForm = (logo: FmsProofLogo): Form => ({
  alt: logo.alt,
  status: logo.status,
...
export default function FmsProofLogoEditPage() {
  ...
  const [logo, setLogo] = useState<FmsProofLogo | null>(null);
  ...
  const save = async () => {
    ...
    try {
       ...
       */
      const body: CreateFmsProofLogoInput = {
      ...
      if (isNew) {
        ...
        toast.success('Logo updated', 'The public FMS page now shows this wall.');
      }
      navigate(LIST_PATH);
    } catch (error) {
