...
const EDIT_PATH = '/cms/products/fms/alternatives-section/rows';

/** MAX_COMPARISON_ROWS on the server. Shown before the 409 fires. */
const MAX_ROWS = 12;

type Pending =
  | { kind: 'delete'; record: FmsAlternativeRow }
  | { kind: 'status'; record: FmsAlternativeRow; next: ContentStatus };

export default function AlternativesRowsCard() {
  const [rows, setRows] = useState<FmsAlternativeRow[]>([]);
  ...
  const filledCount = (row: FmsAlternativeRow) => {
