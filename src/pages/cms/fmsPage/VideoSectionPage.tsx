...
/**
 * The video showcase - "It's Not Just Software — It's Your Franchise Operating
 * System."
 *
...
const EDIT_PATH = '/cms/products/fms/video-section';

/** MAX_FMS_VIDEO_ENTRIES on the server. Shown as a hint before the 409 fires. */
const MAX_ENTRIES = 10;

type StatusFilter = 'all' | ContentStatus;

type Pending =
  | { kind: 'delete'; record: FmsVideoEntry }
  | { kind: 'status'; record: FmsVideoEntry; next: ContentStatus };

export default function FmsVideoSectionPage() {
  const [entries, setEntries] = useState<FmsVideoEntry[]>([]);
    ...
    <>
      <SectionCopyCard
        ...
        placeholders={{
          ...
          heading: "It's Not Just Software — **It's Your Franchise Operating System.**",
          subtext:
      ...
      <ConfirmDialog
        ...
        description={
          pending?.kind === 'delete'
            ...
            : pending?.next === 'ACTIVE'
              ? 'This becomes the video the FMS page plays. Only one can be live, so deactivate the current one first if there is one.'
