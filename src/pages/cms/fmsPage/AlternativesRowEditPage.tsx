 ...
 * use.
 *
 * A row is edited as a whole line: the criterion, then one box per column.
 * Cells are not addressed on their own, so leaving a box empty is how a cell
 * is cleared - the save replaces the row's cells wholesale.
 */
...
export default function FmsAlternativesRowEditPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (isNew) {
          setForm({ parameter: '', status: 'ACTIVE', displayOrder: '', cells: {} });
          return;
        }
        if (!id) return;
        ...
        setForm({
          ...
          cells: Object.fromEntries(found.cells.map((cell) => [cell.columnId, cell.content])),
        });
      } catch (error) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const cells = columns
        .map((column) => ({
          columnId: column.id,
          content: (form.cells[column.id] ?? '').trim(),
        }))
        .filter((cell) => cell.content !== '');

      const body: CreateFmsAlternativeRowInput = {
      ...
      if (isNew) {
        ...
        toast.success('Row updated', 'The public FMS page now shows this grid.');
      }
      navigate(LIST_PATH);
    } catch (error) {
        ...
        description={
          emptyCells > 0
            ...
            : isNew
              ...
              : 'Are you sure you want to update this row? The public FMS page will show it straight away.'
        }
