// src/services/sweetsPageService.ts
...
/**
 * The Sweets & Namkeen industry page, backed by the real API.
 *
...
export const trustSection = {
  /** The icon names a figure may use - exactly what the validator accepts. */
  icons: async (): Promise<string[]> => request<string[]>(`${BASE}/trust-section/icons`),

  logos: listEndpoints<SweetsTrustLogo, CreateSweetsTrustLogoInput, UpdateSweetsTrustLogoInput>(
    `${BASE}/trust-section/logos`,
  ),
  stats: listEndpoints<SweetsTrustStat, CreateSweetsTrustStatInput, UpdateSweetsTrustStatInput>(
...
export const ctaSection = {
  ...
  get: async (): Promise<SweetsCtaSection | null> =>
    request<SweetsCtaSection | null>(`${BASE}/cta-section`),

  save: async (input: UpsertSweetsCtaSectionInput): Promise<SweetsCtaSection> =>
    request<SweetsCtaSection>(`${BASE}/cta-section`, { method: 'PUT', body: input }),
};
