export const env = {
   ...
   *
   * CMS content can store a site-relative image path - the Contact hero is
   * seeded with '/images/contact_us_desktop.webp' - which is one of the
   * website's own assets, not the admin panel's. Previewing one has to resolve
   * it against the site, or the panel requests the path from itself and paints
   * its own index.html into an <img>. See lib/contentUrl.ts (siteAssetUrl).
   *
